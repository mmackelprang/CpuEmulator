...
internal static class CpuEmitter
{
    ...
    private static void EmitMonitorSupport(StringBuilder sb, SpecModel model)
    {
        ...
        sb.AppendLine("        if (t[0] == '(')");
        sb.AppendLine("        {");
        sb.AppendLine("            // All three indirect forms after whitespace strip:");
        sb.AppendLine("            //   ($hh,X)  → IndirectX  (length 7)");
        sb.AppendLine("            //   ($hh),Y  → IndirectY  (length 7)");
        sb.AppendLine("            //   ($hhhh)  → Indirect   (length 7)");
        sb.AppendLine("            // Disambiguated by suffix.");
        sb.AppendLine("            if (t.Length == 7 && t[1] == '$' && t[6] == ')')");
        sb.AppendLine("            {");
        sb.AppendLine("                if (t[4] == ',' && t[5] == 'X' && TryHexByte(t.Substring(2, 2), out lo))");
        sb.AppendLine("                {");
        sb.AppendLine("                    mode = \"IndirectX\";");
        sb.AppendLine("                    return true;");
        sb.AppendLine("                }");
        sb.AppendLine("                if (t[3] == ')' && t[4] == ',' && t[5] == 'Y' && TryHexByte(t.Substring(2, 2), out lo))");
        sb.AppendLine("                {");
        sb.AppendLine("                    mode = \"IndirectY\";");
        sb.AppendLine("                    return true;");
        sb.AppendLine("                }");
        sb.AppendLine("                if (TryHexWord(t.Substring(2, 4), out lo, out hi))");
        sb.AppendLine("                {");
