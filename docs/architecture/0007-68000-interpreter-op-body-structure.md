...
> ### ✅ RESOLVED by Coordinator (2026-06-15)
> **Sub-decision confirmed: the eventual (B) promotion target is a 68000-local generated op-table, NOT literal `EmitOpcodeMethod` reuse.** Rationale: the 68000 is a field-grammar CPU; the row-shaped `EmitOpcodeMethod` (one method per enumerated opcode row) was explicitly rejected for it in the M4.5a plan because it "would force the field-grammar CPU into a row model it does not have." Reusing it would re-introduce exactly the impedance mismatch the field-grammar decode was built to escape. This **refines ADR 0004 §4's sketch** (which predated the field-grammar implementation evidence) to match what the substrate actually became: lifting the (C) descriptor table into the generator as a 68000-local op-table is the **standing direction** for the M4.5c/d promotion. **M4.5b is unaffected — it ships (C).** Revisitable at the actual M4.5c/d promotion if the descriptor-generalization evidence (Open Question #1) argues otherwise.
