...
| **8086** | `BlockCompiler.M8086.cs` | MOV (over the **`(CS<<4)+IP` segmentation seam**); ALU + FLAGS (the **AF/PF** wrinkle); near branch (`Jcc`/`JMP`/`CALL`/`RET`/`LOOP`, near `FF`-indirect); **far flow** (`JMP`/`CALL`/`RET` `9A`/`EA`/`CB`/`CA`/`FF /3`/`FF /5`, over the linear `(CS<<4)+IP` block-cache key — ADR 0019); **`MUL`/`IMUL`/`DIV`/`IDIV`** (`F6`/`F7` /4../7 — ROADMAP #4 row MD; DIV/IDIV vector the divide-error INT0 via the shared `EmitM8086RaiseInterrupt` IVT-push helper and end the block); **string/`REP`** (`MOVS`/`CMPS`/`STOS`/`LODS`/`SCAS` `A4`-`A7`/`AA`-`AF` + `REP`/`REPE`/`REPNE` — ROADMAP #4 row STR; the CX-counted loop + DF-direction + REPE/REPNE ZF early-exit, an in-op back-edge, straight-line); **`INT`/`INT3`/`INTO`/`IRET`** (`CD`/`CC`/`CE`/`CF` — ROADMAP #4 row II; the IVT FLAGS:CS:IP push + IF/TF clear + vector, INTO's OF gate, IRET's pop + reserved-bit forcing `(popped & 0x0FD5) | 0xF002`; every form ends the block, reusing row MD's `EmitM8086RaiseInterrupt`) | `BOUND` (`62`/`63`), `IN`/`OUT`; **AAM/AAD** (`D4`/`D5`) and the divide-error UNDEFINED arithmetic-flag fallout (the corpus classifiers defer these — emit fails-identically-to-the-interpreter) | the segmented EA resolver (`ModR/M` over a segment + offset); the variable-length decode. The block-cache key is the linear `(CS<<4)+IP` (ADR 0019 FF-1), so a CS-changing op (far flow / the divide-error vector / the INT vector) chains/keys correctly. |
