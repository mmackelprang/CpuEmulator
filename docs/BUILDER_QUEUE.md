...
> arm, transcribing `InterruptExecute` + `RaiseInterrupt` (`M8086Cpu.Interrupt.cs:54/39`) one-for-one. INT n/INT3
> advance IP to the return point THEN raise via the **shared `EmitM8086RaiseInterrupt`** (the IVT-push helper Row MD
> introduced — DECISION MD-2, reused here byte-identically); INTO branches on OF (taken → vector 4, not-taken →
> no-op); IRET pops IP:CS:FLAGS with the 8086 reserved-bit forcing (`(popped & 0x0FD5) | 0xF002` — the corpus
> `0x28CF→0xF8C7` case). **Every form changes CS:IP → ENDS the block** (DECISION II-1, endsBlock re-forced like the
> flow + DIV/IDIV rows). **This row was RESUMED** — a prior Builder had claimed II + implemented all six plan tasks
> (the arm + `M8086InterruptEmitTests` + the gate-flip/endsBlock re-force) but the FF-2 negative control + the
> M8088JitTom comment were still uncommitted when it stopped; this session committed those, verified the arm faithful
> one-for-one vs the oracle, ran the `feature-dev:code-reviewer` (all 8 high-risk surfaces clean — the IP-before-raise
> ordering, the IRET pop order + reserved-bit constants `0x0FD5`/`0xF002` verified against `M8086Cpu.Stack.cs`, the
> INTO conditional, the shared-helper reuse, the gate flip + BOUND exclusion, and the FF-2 test split — 0 HIGH/0
> MEDIUM/0 LOW), ran the gates, and shipped. **Un-fakeable gate — GREEN:** (1) **parity** — the full-UAT `M8088JitTom`
> sweep **283/283 byte-identical through the JIT** (the `CD`/`CC`/`CE`/`CF` files now via real emitted IL — the
> pushed FLAGS:CS:IP frame in stack RAM + the vectored CS:IP + IF/TF clear + IRET's reserved-bit forcing); (2)
> **emits-not-fallback discriminator** — `M8086InterruptEmitSelections > 0` AND `FallbackEmitCount == 0` for INT/INTO/
> IRET blocks, with the FF-2 negative control narrowed so **only BOUND (`62`/`63`) stays fallback** (`Bound_stays_fallback`
> + the `Int_into_iret_emit_with_zero_fallback` positive control) — proven red→green. Plus `M8086InterruptEmitTests`
> (INT n frame+vector; INT3; INTO taken/not-taken; IRET `0x28CF→0xF8C7`) + the **shared-helper frame-identity
> cross-check** (`Int0_and_divzero_push_byte_identical_frames` — Row II's INT0 and Row MD's divide-error push
> byte-identical frames, confirming both call the same helper). **Stays fallback by design (noted, not forced):**
> BOUND (`62`/`63`) — out of #4 scope. **Full Release suite green + warning-clean.** AOT-clean Core (all IL in
> `CpuEmulator.Jit`; one generator gate-flip + the additive endsBlock clause; interpreter oracle unchanged).
> **🎉 THE AUTONOMOUS ROADMAP RUN IS COMPLETE — ROADMAP #4 (rows MD ✅ PR #149, STR ✅ PR #150, II ✅ PR #151) is
> SHIPPED; #1/#3/#4/#6/W shipped, #2/#5/L parked. No 📋 queued rows remain. Builder stops per protocol.** Prior
> Builder log:
> **Last updated:** 2026-06-23 (Builder — **STR SHIPPED (PR #150) — the 8086 string/REP family now emits through
> the JIT (ROADMAP #4 finale, row 2 of 3; only II remains).** MOVS (`A4`/`A5`), CMPS (`A6`/`A7`), STOS (`AA`/`AB`),
> LODS (`AC`/`AD`), SCAS (`AE`/`AF`), byte+word, REP/REPE/REPNE-prefixed or not, emit via the new `EmitM8086String`
> arm transcribing `StringExecute`/`StringStep` (`M8086Cpu.String.cs`) one-for-one. SOURCE = `DS:SI` (DS
> override-replaced via the captured prefix); DESTINATION = `ES:DI` (ES non-overridable). The REP path emits the one
> genuinely-new IL shape in #4 — a runtime in-op back-edge `while(CX!=0){CX--; body; if(isCompare && zf!=repWhileZfSet) break;}`
> (REPE `F3` repeat-while-ZF=1 / REPNE `F2` repeat-while-ZF=0; CX=0 → zero iterations). String ops do NOT change CS
> and the loop terminates in-op → **straight-line, no endsBlock re-force** (DECISION STR-1); the body re-resolves the
> EA from live SI/DI each iteration (DECISION STR-2). **This row was RESUMED** — a prior Builder had implemented all
> six plan tasks + the focused tests + the gate un-force (commits `ca292e8`/`14392fa`/`52e6169`, on top of MD's merge)
> but died before claim/review/ship; this session claimed it, ran a FRESH `feature-dev:code-reviewer` pass (all 8
> high-risk surfaces — the REP back-edge, DF-step sign/wrap, ES-non-overridable/DS-override, DataLocal clobber order,
> per-iteration EA, discriminator wiring, CMPS/SCAS SubFlags — verified CLEAN; 0 HIGH / 0 MEDIUM, 1 LOW design note
> kept), and shipped. **Un-fakeable gate — GREEN:** (1) **parity** — the `M8088JitTom` sweep **283/283 byte-identical
> through the JIT** (the `A4`-`A7`/`AA`-`AF` files now via real emitted IL incl. the REP-prefixed cases, not fallback);
> (2) **emits-not-fallback discriminator** — `M8086StringEmitSelections > 0` AND `FallbackEmitCount == 0` over the
> string blocks — the 16 focused `M8086StringEmitTests` (both DF directions; REPE CMPSB early-exit at the exact
> CX/SI/DI; REPNE SCASW match-stop; CX=0 no-op; segment override). **UAT:** the `DifferentialFuzz` JIT-vs-interpreter
> randomized-program sweep **33/33 byte-identical**. **Suite green (foreground slices, no-wedge):** Jit 514, Generators
> 608, M8088JitTom 283, TomHarte-interp 4313, DifferentialFuzz 33 + the lighter namespaces — 0 failed across all
> slices, warning-clean (the full single-process run is split into foreground `--filter` slices because the ~7400-case
> assembly exhausts one test host; each slice runs foreground to completion). AOT-clean Core (all IL in
> `CpuEmulator.Jit`; one generator gate-flip in `IsEmittableX86Family`, NO endsBlock change; interpreter oracle
> unchanged). **Next: row II (INT/INTO/IRET emit) — the last row; the autonomous run ends after it merges.** Prior
> Builder log:
> **Last updated:** 2026-06-23 (Builder — **MD SHIPPED (PR #149) — the 8086 MUL/IMUL/DIV/IDIV ops now emit through
> the JIT (ROADMAP #4 finale, row 1 of 3).** `F6`/`F7` /4 MUL /5 IMUL /6 DIV /7 IDIV (byte+word) emit via the new
> `EmitM8086MulDiv` arm, transcribing `AluMul`/`AluDiv` (`M8086Cpu.Alu.cs:369/412`) one-for-one. MUL/IMUL are
> straight-line (CF=OF on upper-half significance; SF/ZF/PF/AF untouched per the interpreter); DIV/IDIV are
> fault-capable — divide-by-zero OR quotient-overflow (incl. the 8086 IDIV symmetric-range quirk `|quot|>127`/
> `>32767`) vectors INT0 via the new shared **`EmitM8086RaiseInterrupt`** IVT-push helper (DECISION MD-2, **owned
> here**, reused by row II), so **DIV/IDIV END the block** (DECISION MD-1, endsBlock re-forced like the flow rows;
> MUL/IMUL do not). **This row was RESUMED** — the prior Builder had implemented all six plan tasks + completed the
> `feature-dev:code-reviewer` pass (the `EmitM8086RaiseInterrupt` CLOBBERS contract + the MUL/DIV mayWriteRam-exclusion
> note, commit `1e4e8e8`) but died on a transient API error before the final suite-green + PR + merge; this session
> verified the implementation faithful (one-for-one vs the oracle), ran the gates, and shipped. **Un-fakeable gate —
> GREEN:** (1) **parity** — the full-UAT `M8088JitTom` sweep **283/283 byte-identical through the JIT** (the `F6`/`F7`
> files now via real emitted IL, not fallback; the divide-error/IDIV-quirk classifiers still defer the same cases
> because emit == interpreter); (2) **emits-not-fallback discriminator** — `M8086MulDivEmitSelections > 0` AND
> `FallbackEmitCount == 0` over a MUL block + a non-faulting DIV block + the divide-by-zero INT0 frame byte-identical
> (full registers + the 6-byte pushed FLAGS:CS:IP stack frame memcmp) to a fresh interpreter — the 8 focused
> `M8086MulDivEmitTests`. **Full Release suite 7406 passed / 0 failed / 4 skipped, warning-clean** (+8 vs the FF-2
> baseline of 7398 — the new MD tests; the 4 skips are the pre-existing WozDiskFact + SoftCard-detect + Klaus
> JIT/SMC asset gates). **Stays fallback by design (noted, not forced):** BOUND (`62`/`63`), AAM/AAD (`D4`/`D5`) — out
> of #4 scope — and the divide-error UNDEFINED arithmetic-flag fallout (DD6) — emit reproduces the interpreter's
> *modeled* behavior exactly, so the existing classifiers keep deferring the same cases. AOT-clean Core (all IL in
> `CpuEmulator.Jit`; one generator gate-flip in `IsEmittableX86Family`; interpreter oracle unchanged). **Next:
> Builder picks up STR** (the 8086 string/REP emit — `A4`-`A7`/`AA`-`AF` + REP/REPE/REPNE; the in-op back-edge loop;
> no deps). Prior Planner log:
> **Last updated:** 2026-06-23 (Planner — **ROADMAP #4 PLANNED — the autonomous-run FINALE (rows MD, STR, II).**
> The owner chose to ship deferred item **#4** (8086 MUL/DIV + string/REP + INT/IRET emit) and PARK the rest of the
> tail (#2 68000 timing, #5 per-bank/generic-emitter, L). Grounded against fresh `main` @ `4b46da2` (the FF-1/FF-2
> far-flow arc is complete — the linear `(CS<<4)+IP` block key is live). The three op groups are **fallback by
> design today** (ADR 0011 §2 — rare / high-emit-cost, NOT correctness); #4 emits them as a pure perf dial, the
> interpreter staying the oracle + byte-exact fallback. **Three bite-sized TDD rows, owner-priority order
> (independent — disjoint opcode sets; ship in order, could interleave):** **MD** (MUL/IMUL/DIV/IDIV `F6`/`F7`
> /4../7 — MUL/IMUL straight-line, DIV/IDIV fault-vector INT0 → end the block; owns the shared `EmitM8086RaiseInterrupt`
> IVT-push helper), **STR** (MOVS/CMPS/STOS/LODS/SCAS `A4`-`A7`/`AA`-`AF` + REP/REPE/REPNE — the CX-loop + DF-direction
> + ZF early-exit; the one new IL shape, an in-op back-edge; straight-line), **II** (INT/INT3/INTO/IRET `CD`/`CC`/`CE`/
> `CF` — the IVT push/vector + IRET pop, reusing MD's helper; every form ends the block). **Each row's un-fakeable
> gate = parity (the `M8088JitTom` corpus byte-identical through the JIT — already green via fallback, now via real
> emitted IL) PLUS an emits-not-fallback discriminator** (a new `M8086{MulDiv|String|Interrupt}EmitSelections > 0`
> AND `FallbackEmitCount == 0` for the targeted opcodes — proven red→green, so the parity sweep can't false-pass on
> a silent still-fallback). **Corners that STAY fallback by design (noted, not forced — ADR 0011 §2/OQ5):** BOUND
> (`62`/`63`), AAM/AAD (`D4`/`D5`) — both out of #4 scope; and the divide-error UNDEFINED arithmetic-flag fallout
> (DD6) — emit reproduces the interpreter's *modeled* behavior exactly, so the existing divide-error/IDIV-sign-quirk
> corpus classifiers keep deferring the same cases. AOT-clean Core (all IL in `CpuEmulator.Jit`; per-row generator
> gate-flips in `IsEmittableX86Family`; interpreter oracles unchanged). Plans:
> `superpowers/plans/2026-06-23-8086-muldiv-emit.md`, `…-string-rep-emit.md`, `…-int-iret-emit.md`; spec
> `superpowers/specs/2026-06-23-8086-muldiv-string-int-emit-design.md`. **Builder clears MD → STR → II; the
> autonomous run ends after II merges.** Prior Builder log:
> **Last updated:** 2026-06-23 (Builder — **FF-2 SHIPPED (PR #147) — the 8086 far transfers now emit through the
> JIT; the ADR 0019 far-flow arc is COMPLETE.** The far `CALL` (`9A` direct / `FF /3` indirect), far `JMP` (`EA`
> direct / `FF /5` indirect), and far `RET` (`CB` / `CA`-imm16) emit byte-identical to the interpreter; the direct
> forms chain across the segment change to the projected key `((newCS<<4)+newIP)&0xFFFFF` (the FF-1 payoff).
> `INT`/`INTO`/`IRET`/`BOUND` stay fallback (ADR 0019 Decision 3). New machinery: `EmitM8086SetCs`/
> `EmitM8086SetCsFromStack`, the `EmitM8086FarFlow` arm (runs before near; `false` for `FF /2 /4`), `EmitM8086LoadFarPtr`
> (`m16:16`), and the `IsEmittableX86FarFlow` generator un-force. The far frame follows ADR 0005 (CALL pushes
> CS-then-IP, RET pops IP-then-CS). **⛔ SEQUENCING HONORED: FF-1 (the linear key) landed + passed its byte-for-byte
> identity gate at PR #146 BEFORE this PR opened — FF-1 and FF-2 were NEVER co-merged** (the aliasing bug only arms
> once an emitted op changes CS mid-chain, so the key had to be live + proven first). **Un-fakeable gates GREEN:**
> (1) the **far-transfer aliasing regression** — two segments at the same IP offset (phys 0x10100 vs 0x20100) compile
> to DISTINCT blocks and each runs ITS OWN code (a segment-unique side-effect byte), **proven red→green** (under a
> temporary bare-IP key segment B's marker is absent + its linear key uncached — the corruption bug; with the linear
> key both markers present + both keys cached + the bare-IP key absent); (2) **far-flow TomHarte-through-JIT
> byte-identity** — the existing `M8088JitTom` corpus (283 cases) sweeps `9A`/`EA`/`CB`/`CA`/`FF.3`/`FF.5` through the
> JIT, all byte-identical to the interpreter (registers + FLAGS + CS:IP + the far stack frame + memory); (3)
> `FallbackEmitCount` drops by exactly the far opcodes + `INT`/`INTO`/`IRET`/`BOUND` stay fallback (gated). The
> TomHarte gate caught a real prefix-scan bug during implementation (segment-override prefixes shifted operand reads)
> — fixed in the arm, not the test. **Pre-merge review (`feature-dev:code-reviewer`): verdict correct; all seven
> high-risk surfaces clean (far frame order, prefix scan, chain-vs-exit, FF-group dispatch, generator un-force,
> aliasing genuineness, no-flags); 1 MEDIUM fixed (a `MakeCompiler` test passed the bare IP instead of the linear key
> to `Compile`), 1 LOW correctly skipped (the runtime `OpcodeDescriptor` has no SubField, so the gate can't tighten
> — the over-admit is harmless by design).** The reviewer specifically verified the dropped `mod==3` special-case for
> `FF /3 /5` is byte-identical to the interpreter (both treat it as a register-based memory EA). **Full Release suite
> 7398 passed / 0 failed / 4 skipped, warning-clean** (+20 vs FF-1 — the far-flow + aliasing gates). **🎉 THE FIVE
> PLANNED QUEUE ROWS ARE CLEARED — W, D68, B68-DOC, FF-1, FF-2 all ✅. No 📋 rows remain** (row L is ⏸️ deferred +
> unplanned). Builder stops per protocol; the architectural items (#2 68000 timing, #4 8086 MUL/DIV, #5 generic
> emitter, L) are NOT yet planned — the Planner/Coordinator/Architect handle the next round. Prior Builder log:
> **Last updated:** 2026-06-22 (Builder — **FF-1 SHIPPED (PR #146) — the SAFE-gated JIT block-cache key widening
> is LIVE; the far arms (FF-2) are now sound to add.** ADR 0019 FF-1: the shared block-cache key is widened
> `ushort → uint` to the generic linear `(CS<<4)+IP` across `BlockCache`/`ChainTable`/`CompiledBlock`/`JittedCpu`/
> `BlockCompiler`, plus the additive `uint IJitTarget.ProjectBlockKey(ICpuCore)` (8086 folds `((CS<<4)+IP)&0xFFFFF`;
> the non-segmented 6502/Z80/68000 project the identity `(uint)PC`). The existing 8086 near-flow static chain edges
> are folded through the baked code-phys base so the chain-edge key equals the dispatch-time projection.
> **NO far op emitted** (`9A`/`EA`/`CB`/`CA`/`FF /3`/`FF /5` stay fallback — that's FF-2); `_blocksByPage` + the
> dirty-page/SMC machinery untouched. **The load-bearing SAFE merge gate is GREEN:** the byte-for-byte
> non-segmented identity regression (`KeyWideningIdentityTests`) — the flat CPUs' block counts, chains, evictions,
> recompiles, and `FallbackEmitCount` are byte-identical to the pre-FF-1 baselines (captured on deterministic
> synthetic workloads that genuinely compile/chain/evict — the reviewer verified the gate is non-vacuous, not
> circular). Plus `ProjectBlockKeyTests` (identity + 8086 fold + overlapping-segment coherence) + the 8086
> near-chain pin (red→green) + all four CPUs' TomHarte-through-JIT parity. **Pre-merge review
> (`feature-dev:code-reviewer`): verdict correct + mergeable; 1 MEDIUM fixed (a stale `(ushort)PC` docstring in
> `BlockCache.FlushAll`), 1 PRE-EXISTING gap logged for the Planner (the 8086 intra-block SMC guard's spanned-page
> set is IP-space vs the physical-space dirty-mark — broken under non-zero CS, predates FF-1, harmless for shipped
> single-step 8086 workloads, doesn't affect the flat CPUs; logged as a candidate in ROADMAP).** **One real FF-1
> regression caught by the full suite + fixed:** the generator's new `public uint ProjectBlockKey(...)` method
> tripped an over-broad 8/16-only additivity guard (`SyntheticWideRegisterByteIdentityTests` asserted no
> `public uint ` token at all); the test was about register FIELDS, so it was tightened to forbid a `uint` *field*
> (`public uint <name>;`) while allowing the `uint`-returning seam method — its stated intent. **Full Release suite
> 7378 passed / 0 failed / 4 skipped, warning-clean.** **⛔ NEXT: FF-2 (the far emit arms) is now eligible — FF-1
> has landed + passed its identity gate, the precondition ADR 0019 §5 requires. FF-2 is a SEPARATE PR and was
> NEVER co-merged with FF-1.** Prior Builder log:
> **Last updated:** 2026-06-22 (Builder — **B68-DOC SHIPPED (PR #145) — roadmap #3 closed truthfully (doc-only).**
> The two stale claims in the roadmap's 68000 bench-cleanups item are reconciled: (a) the **W3 hot-op profiler arm
> shipped** in `bc68ee7` (`Profiler.cs:215` `Run68000("W3 sieve-kernel", …)`; the `## 68000 — W3 sieve-kernel`
> block is in `bench/hotop-profiler/hotop-profile-results.txt`, the `m68k-W3` throughput rows in
> `bench/results/REPORT.md`); (b) the **W2 cycle off-by-2 is accepted coarse-cycle slack** per **DECISION T2**
> (the JIT charges coarse `BaseCycles + 1` fetch vs the interpreter's exact prefetch — gated within a `<= 16`
> tolerance in `BenchHarnessSmokeTests`, not a bug). Retagged `docs/ROADMAP.md` item #3 `[deferred]`→`[resolved]`
> (kept in-list per house style) + reframed the `docs/user-guide/benchmarks.md` caveat. **Un-fakeable gate:** a
> self-pinning doc-assertion test (`BenchDocReconciliationTests`, 3 facts) — stale phrases gone (scoped to the two
> target docs only — the plan/spec files that quote the old text are untouched), citations present, and the cited
> evidence still exists in source (the `Run68000` call, the hot-op block, the REPORT.md rows, the W2 `<= 16` gate).
> Proven red→green against the pre-edit docs. **Pre-merge review (`feature-dev:code-reviewer`): 1 HIGH — fixed.**
> The HIGH was an inaccurate file citation: the draft said the `## 68000 — W3 sieve-kernel` block was in
> `REPORT.md`, but that exact heading lives in `hotop-profile-results.txt` (REPORT.md carries the `m68k-W3`
> *throughput* rows); corrected in both docs + pinned in the gate so it can't drift back. **Full Release suite
> green + warning-clean** (doc-only + one assertion test; no CPU/JIT/bench/harness code touched). **Next: the
> ⛔-sequenced FF-1 → FF-2 pair** (ADR 0019 8086 far-flow; FF-1 the linear `(CS<<4)+IP` block key + the
> `ProjectBlockKey` seam, then FF-2 the far emit arms — FF-1 must land + pass its identity gate BEFORE FF-2 opens,
> NEVER co-merged). Prior Builder log:
> **Last updated:** 2026-06-22 (Builder — **D68 SHIPPED (PR #144) — the `--board 68000` monitor renders real
> mnemonics, not `???`.** Roadmap deferred #6 closed: the field-grammar 68000 had no `InstructionDef` rows, so the
> generated `Disassemble` was a `???`-only stub. Added a **`FieldGrammar`-gated** branch to the generator's
> `EmitDisassembler` (`EmitM68kDisassembler`) that emits an unrolled if-chain over `Decode68k.Ops` (grammar order,
> first `(opword & Mask)==Match` wins — exactly like the decoder) rendering mnemonic family + size suffix
> (`.B`/`.W`/`.L`) + condition code + EA operand; **all 83 grammar families wired.** Extension-word operands render
> honest placeholders (`#<imm>`/`<abs>`/`<d16>`) — the 3-byte `IMonitorSupport.Disassemble` contract is NOT widened
> (D68-3); in-opword operands (registers, EA mode, size, cc, MOVEQ's signed imm, ADDQ/SUBQ quick, TRAP vector)
> render fully. **Monitor-display-only: no IL, no execution-path change** — TomHarte/decode/JIT-parity untouched.
> **Pre-merge review (`feature-dev:code-reviewer`): encoding verified correct across all 30 sampled families (no
> wrong mnemonic/size/cc/register/EA); 1 IMPORTANT functional + 1 comment nit — both fixed.** The IMPORTANT was a
> monitor-walk mis-alignment: the generated `InstructionLength(byte)` returned the `Undefined` fallback of 1 for
> the word-granular 68000 (the byte key can't resolve its compound operation key), so a multi-instruction
> `d <addr> <count>` walk advanced 1 byte/step, landing every instruction after the first on the previous opword's
> low byte; fixed (FieldGrammar-gated) to return the 2-byte opword minimum so the walk stays opword-aligned, with a
> new red→green gate (`The_m68000_monitor_walks_multiple_instructions_opword_aligned`). **Un-fakeable gates GREEN:**
> the curated round-trip corpus (each row fails on a wrong family/size/cc/EA); the decoder-agreement sweep (all
> 65536 opwords — the **54,071** the grammar recognizes render non-`???`, read straight from the shipped
> `Decode68k.Ops`); the monitor-host gates. **DECISION-R regression:** the branch is `FieldGrammar is not null`-gated
> + returns before the `Instructions` switch, so the 6502/Z80/8086 generated `Disassemble`/`InstructionLength` are
> byte-for-byte unchanged (their disasm tests stay green). **UAT (Builder, live):** ran the actual
> `dotnet run --project src/CpuEmulator.Host -- --board 68000` monitor and disassembled the boot ROM — it renders
> `MOVEQ #79,D0` / `MOVE.B D0,<abs>.L` / `MOVEQ #75,D0` (the `O`=0x4F, `K`=0x4B "OK" message bytes), opword-aligned
> (`0008→000A→000C→…`, each +2). No `???` for real code. **Full Release suite 7353 passed / 0 failed / 4 skipped,
> warning-clean** (+20 vs the W baseline — the new M68k disasm gates; the 4 skips are the pre-existing Klaus×2 +
> SoftCard-detect + Woz-headline asset gates). **Next: Builder picks up B68-DOC** (the bench doc-reconciliation,
> doc-only — no deps). Prior Builder log:
> **Last updated:** 2026-06-22 (Builder — **W SHIPPED (PR #143) — `.woz` now works end-to-end.** `WozFluxImage`
> parses a WOZ2 `.woz` container (INFO/TMAP/TRKS → per-track MSB-first bitstreams + `bit_count` loop length) into
> the shipped `IFluxImage` seam the `Apple2DiskII` head reads identically to `Dsk`/`SyntheticFluxImage` — plus a
> `src`-resident standard CRC32 (`WozCrc32`), the `WozAsset` loader + `get-woz-disks.{sh,ps1}`, and the three
> one-line surface wirings that flip `.woz` on in `DiskImageFactory.FromBytes` (was `NotSupportedException`),
> `UploadValidator` (was honest-reject), and `DiskCatalog` (was `Supported:false`). **WOZ2 only** (W-1, WOZ1
> rejected), **5.25" only** (W-6), **read-only** (W-3), absolute `starting_block*512` addressing, `TMAP[t*4]`
> whole-track resolution (W-7). **Pre-merge review (`feature-dev:code-reviewer`): 1 CRITICAL / 1 HIGH / 2 MEDIUM —
> all fixed.** The CRITICAL was a genuine functional gap the implementer missed: `Program.cs`'s library-insert
> handler still wrapped the insert in `if (format != DiskFormat.Woz)` (a no-op guard from when `.woz` was
> unsupported) → selecting a `.woz` from the library dropdown was a *silent no-op*; removed the guard AND added
> `InvalidDataException` to the swallow-filter so a malformed library `.woz` can't tear down the live WS session.
> The HIGH was a robustness leak — an out-of-range TMAP byte (160–254) indexed the 160-entry `_trk` and threw
> `IndexOutOfRangeException` instead of the contract's `InvalidDataException`; added a bounds guard. Two MEDIUM
> stale comments (Program.cs/app.js) refreshed. **Un-fakeable gates GREEN:** asset-free — a hand-built WOZ2
> round-trips track bits + `bit_count`; wrong-CRC32 / WOZ1 / non-5.25" rejected; TMAP resolves `t*4`; the factory
> builds a `WozFluxImage` from `.woz` bytes. The asset-gated `[WozDiskFact]` headline gate skips-with-note (no
> public-domain WOZ2 locatable — decision **W-8** fallback; the asset-free gates carry the row). **UAT (Builder,
> asset-free + live):** re-nibblized a synthetic DOS 3.3 image into a real track-0 bitstream (carries genuine
> `D5 AA 96` address prologues), wrapped it in a valid WOZ2, parsed it back through production `WozFluxImage`, and
> drove it through the LIVE `Apple2DiskII` head — the `D5 AA 96` prologue is found; ALSO seeded that real-shaped
> WOZ2 into the cache and ran the actual asset-gated headline gate **LIVE (not skipped) → PASS**, then removed the
> synthetic asset so the gate returns to honest skip-with-note (no synthetic asset masquerading as real). **Full
> Release suite 7333 passed / 0 failed / 4 skipped, warning-clean** (the +1 net skip is the new `[WozDiskFact]`;
> the other 3 are the pre-existing Klaus×2 + SoftCard-detect asset gates). **Next: Builder picks up D68** (the
> 68000 disassembler — no deps). Prior Planner log:
> **Last updated:** 2026-06-22 (Planner — **ADR 0019 8086 FAR-FLOW ARC PLANNED (rows FF-1, FF-2)** — the
> TDD decomposition of ADR 0019 (`architecture/0019-8086-far-flow-emit-and-the-cs-ip-block-key.md`, classified
> **SAFE** by the Architect), grounded against fresh `main`. Two strictly-ordered rows: **FF-1** the
> `ushort→uint` linear `(CS<<4)+IP` block-cache key + the per-CPU `IJitTarget.ProjectBlockKey` seam (8086 folds
> the segmented origin; the non-segmented 6502/Z80/68000 project the **identity `(uint)PC`** → byte-for-byte
> unchanged), gated by the **key-projection identity regression** (the SAFE proof) — **NO far op emitted**; then
> **FF-2** the far `JMP`/`CALL`/`RET` emit arms (CS-load + doubled far push/pop per ADR §4; `INT`/`INTO`/`IRET`/
> `BOUND` stay fallback), gated by the **aliasing regression** (two segments / same offset → distinct blocks —
> FAILS on the old `(IP)` key, PASSES on the linear key) + far-flow TomHarte-through-JIT byte-identity.
> **⛔ CRITICAL SEQUENCING (ADR 0019 §5): FF-2 depends on FF-1, and they MUST NOT be co-merged** — the aliasing
> bug only arms once an *emitted* op changes CS mid-chain, so FF-1 (the key) must land + pass its identity gate
> before FF-2 (the far arms) is sound. Plans: `superpowers/plans/2026-06-22-ff1-linear-block-key-and-projectblockkey-seam.md`,
> `superpowers/plans/2026-06-22-ff2-far-flow-emit-arms.md`. **Builder clears W → D68 → B68-DOC → FF-1 → FF-2.**
> Prior Planner log:
> **Last updated:** 2026-06-22 (Planner — **THREE roadmap follow-ons PLANNED (rows W, D68, B68-DOC)** — grounded
> against `main` @ `7cb3265`. All three are additive seam-consumers (no cross-cutting architecture; no Architect
> gate). **Priority order W → D68 → B68-DOC** (all independent — no inter-deps; B68-DOC has zero code-merge
> surface and can interleave anytime). **W — `WozFluxImage`** ([plan](superpowers/plans/2026-06-22-woz-flux-image.md),
> dep F ✅): the thin WOZ2 `.woz`-file parser → `IFluxImage` (the seam match is direct — WOZ2 TRKS is MSB-first
> packed bits + a `bit_count` loop length, exactly `TrackBits`/`TrackBitLength`), wired into
> `DiskImageFactory`/catalog/upload so `.woz` insert/upload/library work end-to-end. Un-fakeable: a real
> fetch-on-demand (never-vendored) `.woz`'s CRC32 verifies on real bytes + its track-0 bitstream reads on the
> **live `Apple2DiskII` head** (`D5 AA 96` prologue); asset-free round-trip/CRC32/TMAP/rejection gates carry the
> row if no public asset is locatable (W-8). WOZ2 only (W-1), 5.25" only (W-6), read-only (W-3). **D68 — 68000
> disassembler** ([plan](superpowers/plans/2026-06-22-m68000-disassembler.md), no deps): roadmap #6 — the
> field-grammar 68000's generated `Disassemble` is a `???` stub (no `InstructionDef` rows); add a
> `FieldGrammar`-gated `EmitM68kDisassembler` walking `Decode68k.Ops` → mnemonic + size + cc + EA, with honest
> extension-word placeholders (the 3-byte `IMonitorSupport` contract is NOT widened — D68-3). **Monitor-display
> only: no IL, no execution-path change** (TomHarte/decode/JIT-parity untouched). No 68000 assembler (D68-1) → a
> curated round-trip corpus; plus a decoder-agreement sweep + a monitor-host gate; DECISION-R empty-diff on the
> other three CPUs. **B68-DOC — bench reconciliation**
> ([plan](superpowers/plans/2026-06-22-bench-doc-reconciliation.md), no deps): **doc-only**, closes roadmap #3 —
> the W3 profiler arm shipped (`bc68ee7`) + the W2 off-by-2 is accepted coarse-cycle slack (DECISION T2, gated in
> `BenchHarnessSmokeTests`); edits `ROADMAP.md`/`benchmarks.md` + a self-pinning doc-assertion gate. Specs under
> `docs/superpowers/specs/2026-06-22-*`. **Next: Builder picks up W** (topmost, dep F ✅). Prior Builder log:
> **Last updated:** 2026-06-22 (Builder — **🎉 THE HEADLINE SHIPPED — CP/M 3.1 BOOTS TO `A>` IN 80 COLUMNS ON THE
> VIDEX (PR #139 → the complete V80-1/2/3/4 sub-arc).** The fifth-layer blocker is FIXED: ADR 0018-C's **two-latch
> 74LS175 Language-Card write-enable correction** (classified SAFE, live-proven). The real apl2cpm3 Disk 1 now boots
> all the way to the decoded **`A>`** CCP prompt on the Videx 80-col VRAM, `CoprocessorActive`, and the
> DisplayMultiplexer **auto-engages the Videx (`ActiveIndex==1`)** — the first true 80-col CP/M end-to-end. **V80-4
> (the LC fix):** `Apple2LanguageCard.Access` now models the real 74LS175 TWO latches (MAME `ramcard16k` `do_io` /
> Sather ch.5) — an **even** access clears both the pre-write count AND write-enable; an **odd-address WRITE** clears
> only the count (write-enable, if already armed, SURVIVES — the bug: the old single-flag model wrongly
> write-protected LC bank 2 on apl2cpm3's `LD ($C08B),A` bank-2 select, dropping the `?ldccp` `LDIR` CCP copy → bank 2
> zeroed → `RET` into a zeroed `$1901`); an **odd-address READ** arms/enables (two consecutive). Single method, no
> signature/seam change. **V80-3 (the auto-engage):** the live boot pin confirmed apl2cpm3 programs the Videx CRTC
> (`$C0B1` data writes: 420) but NEVER does a `$C0B8-$C0BF` bank-select (0) — so the old bank-select-only engagement
> trigger never fired. `VidexVideoterm.WriteReg` now also engages on a **CRTC-data write** (`$C0B1` — the firmware
> bringing the 80-col display online; the no-op guard fires `ActiveChanged(true)` exactly once), so `ActiveIndex`
> flips to 1 from a real CP/M-3 boot. **SAFE-classification regression guard — all GREEN live:** the 2.2 **CPM-4
> committed-hash `A>` gate** + **CPM-5 Videx `ActiveIndex==0` gate** (the 2.2 master issues ZERO `$C0Bx` so the new
> trigger never fires — `ActiveIndex==0` byte-for-byte), all **13 LC unit tests** (incl. the new two-latch lock),
> and the full Apple2/SoftCard/dual-CPU sweep. **Gates strengthened:** `[Apl2Cpm3VidexFact]` from the sign-on
> substring → the **decoded `A>`** off the live `$CC00` VRAM + a **LC-bank-2-nonzero discriminator** (3026/4096 bytes
> post-LDIR, 0 under the old model) + the **positive LC unit test** (an odd write preserves write-enable). **Pre-merge
> review (`feature-dev:code-reviewer`): 0 HIGH / 1 MEDIUM / 0 LOW — fixed** (stale BootProbe "does not reach `A>`"
> comments refreshed). **Full Release suite 7325 passed / 0 failed / 3 skipped, warning-clean** (the V80-3 test fix
> turned the one Videx-test regression green; the lone WebServerSmokeTests FB-frame-ordering flake is the documented
> pre-existing demo-WS race — passes 5/5 in isolation, never enters `RunDualCpu`). **Headline screenshot:
> `/d/prj/cpm-videx-80col-A-LIVE.png`** (`CP/M Version 3.0, 56K BIOS R6/89` / `A>` / `46K TPA`; frame SHA-256
> `627a1657…5004`). **The whole 80-column CP/M arc is DONE — V80-1/2/3/4 ✅.** Prior partial-milestone log:
> **Last updated:** 2026-06-22 (Builder — **V80-2/V80-3 PARTIAL SHIPPED — the genuine CP/M 3.1 console renders on
> the Videx 80-col card; `A>` blocked by a NEWLY-PINNED fifth layer (a Language-Card banking write-protect defect).**
> Three layers cleared live on the real apl2cpm3 Disk 1 (slot-4 SoftCard+Videx board): **(1)** `SectorOrderKind.Cpm3`
> = **raw DOS 3.3 on EVERY track** (ADR 0018-A live-resolved — NOT the plan's per-track split: apl2cpm3's BOOTLDR
> `xlt` + the running LDRBIOS `fdxlt` compose to identity over a raw presentation; under it CPMLDR's `LD SP` (`$31`)
> lands at Z80 `$0100` byte-exact, `CPM3.SYS` reads byte-exact); **(2)** the **`?jsr65` Z80↔6502 service-loop bridge
> round-trips with ZERO machine change** — the live boot shows ~73 hand-backs, so **ADR 0018-B's predicted dead
> bridge is FALSIFIED** (the natural `$03C9` resume already re-enters `L65A`; setting a `$03C0`/`$03C6` resume entry
> BREAKS it — so NO bridge plumbing was added, contradicting the V80-2 plan Task 2b); **(3)** with the **REAL Videx
...
> 0 LOW — all fixed.** Full Release suite green + warning-clean; the load-bearing **2.2 CPM-4 hash + CPM-5 Videx
> `ActiveIndex==0` gates ran LIVE and passed**. **THE WALL (filed as a Blocker below, NOT faked): `A>` is NOT
> reached.** After the sign-on the CCP runs and the banked CP/M-3 BDOS hits a **deterministic** divergence — a
> `RET` into a **zeroed** Z80 `$1901` (phys `$2901`, never written; reproduced byte-identically at instr 36583,
> `PC=$1929`). Diagnostic-classified **BUCKET A (a load/banking defect, NOT a Z80-core/translation mis-execution)**:
> the CP/M-3 loader's `?ldccp` copies the CCP into **Language-Card bank 2** via `LD ($C08B),A` (select bank 2) →
> `LDIR` → `LD ($C083),A`; our `Apple2LanguageCard.Access` treats the bank-select **write** to `$C08x` as forcing
> `_writeEnabled=false`, so bank 2 is remapped write-protected and the `LDIR` writes are **silently dropped** —
> bank 2 stays zeroed → the later banked pointer chain lands on the zeroed `$1901`. The fix is a **shared
> Language-Card write-enable-truth-table change** (used by DOS/Applesoft/2.2) — a risky change outside V80-2's
> apl2cpm3-scoped seams that ADR 0018-A A1 ruled off-limits, so **Builder STOPPED per protocol and filed the
> Blocker for the Architect** (likely an ADR 0018-C + a new queue row V80-4). **So V80-2 is shipped as the proven
> sign-on milestone (not ✅ `A>`); V80-3's `ActiveIndex==1` + the 80-col `A>` headline stay OPEN behind the LC
> blocker.** Prior Builder log:
> **Last updated:** 2026-06-22 (Builder — **V80-1 SHIPPED (PR #137) — the gating fix is live-verified on the real
> apl2cpm3 disk.** The SoftCard control-port slot is now a **per-board parameter**: `SoftCardBoard.Spec` /
> `SoftCardVidexBoard.Spec` take an optional `controlPortBase` **defaulted to slot 5 / `$C500`** (the shipped 2.2
> board is byte-for-byte unchanged — `ControlPortBase` kept as an alias to `ControlPortBaseSlot5`); apl2cpm3
> passes slot 4 / `$C400`. Added the `Apl2Cpm3` asset loader (distinct `cpm/apl2cpm3/CPM3.1_Disk_1.dsk` cache
> path — never clobbers the 2.2 `cpm/softcard-cpm.dsk`) + `tools/get-apl2cpm3.{sh,ps1}` (fetch-on-demand,
> sign-off covered by the existing CP/M-disk sign-off) + the apl2cpm3 40-col boot gate **present but
> named-skipped** until V80-2 (never false-passing). **Un-fakeable slot-placement gate** (asset-free, 4
> `[Theory]` rows): a `$C400` write toggles the slot-4 board's coprocessor, a `$C500` write does not, and the
> mirror for the default board — the `false` rows would only pass under a both-slots fiction (which the code
> rejects). The slot-4 board validates+builds (`$C400` contained in the `$C000-$C5FF` Mmio region); the guard
> also rejects the `$C600` disk-boot-ROM page (pre-merge fix — the guard's comment had falsely claimed
> validator-cleanliness for any in-band slot). **Pre-merge review (`feature-dev:code-reviewer`): 0 HIGH / 1
> MEDIUM / 2 LOW — all fixed** (the disk-boot-ROM-window guard gap + the `$C000-$C7FF`→`$C000-$CFFF` doc fix +
> a vacuous path-isolation `NotEqual`-vs-null strengthened to direct `Contains`/`DoesNotContain`). **Live UAT
> (the un-fakeable proof V80-1's change is correct):** booting the real apl2cpm3 Disk 1 on the **slot-4** board
> → the Z80 **activates** (`CoprocessorActive=True`, screen `APPLE ][`, no error); on the **slot-5** default
> board → the Z80 **never activates** and the screen prints **`NO Z80 FOUND`** — exactly the failure mode ADR
> 0018 traced. (V80-1 correctly does NOT reach `A>`: the Z80 still NOP-slides from `$0000` because the entry
> handoff is absent — that is V80-2.) Full Release suite **7322 passed / 0 failed / 4 skipped**, warning-clean;
> the load-bearing **2.2 no-regression gates ran LIVE and passed** (CPM-4 committed-hash `A>` gate + CPM-5
> Videx gate — the slot default is unchanged, so the 2.2 board is byte-for-byte identical). The apl2cpm3 gate
> is the +1 net skip. **Next: Builder picks up V80-2** (CP/M 3.1 `A>` in 40-col — close the Z80-entry handoff
> via live triage; ⚠ pause + report if it needs a non-`$0000` Z80 start PC latched — the flagged Z80-reset
> risk). Prior Planner log:
> **Last updated:** 2026-06-22 (Planner — **80-column CP/M sub-arc PLANNED (new rows V80-1…V80-3)** — apl2cpm3
> CP/M 3.1 on the SoftCard + Videx, grounded against `main` @ `7dacd6d` (the commit carrying **ADR 0018**). The
> Architect root-caused the never-booting apl2cpm3 to **one gating change** (live-verified on the real Disk 1):
> the SoftCard control port must decode at **slot 4 (`$C400`)**, not the shipped 2.2 slot 5 (`$C500`) — apl2cpm3
> hard-codes `STA $C400` to start the Z80 (re-verified this session: `8D 00 C4` at track-0 offset `$DBA` of the
> staged `cpm/videx-candidates/apl2cpm3-disk1.dsk`; the disk's ASCII carries `CP/M V3.0 Loader` / `Copyright (C)
> 1982, Digital Research` / `CPMLDR COM` / `CPM3.SYS`). Three strictly-ordered bite-sized TDD plans:
> **V80-1** ([plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr1.md), no deps): the **configurable per-board
> SoftCard slot** (`SoftCardBoard.Spec`/`SoftCardVidexBoard.Spec` gain an optional `controlPortBase`, **defaulted
> to slot 5 so the 2.2 board is byte-for-byte unchanged**; apl2cpm3 → slot 4) + the `Apl2Cpm3` asset loader
> (distinct `cpm/apl2cpm3/` cache path — never clobbers the 2.2 disk) + `get-apl2cpm3.{sh,ps1}` + an honest
> **named-skipped** boot gate. Un-fakeable: a slot-placement run-loop test (`$C400` toggles the slot-4 board,
> `$C500` does not) + the full 2.2 suite green (the CPM-4 hash + CPM-5 Videx gates unchanged). **V80-2**
> ([plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr2.md), dep V80-1): **CP/M 3.1 `A>` in 40-col** — close
> the Z80-entry handoff (the loaded CPMLDR entry at `$1100+`) via **live triage** against the slot-4 disk (the
> ADR 0017 `$1010`-bridge discipline; likely **outcome (1): no production change**). **⚠ Flagged risk** (ADR
> 0018 D6/OQ1): if the boot needs a **non-`$0000` Z80 start PC** latched (vs a loader `JP` stub), that touches
> the Z80-core reset path — judged unlikely; the Builder **pauses + reports**. Gate: decoded `A>` + a CP/M-3
> sign-on substring + `CoprocessorActive` on the live disk. **V80-3**
> ([plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr3.md), dep V80-2): **THE HEADLINE — CP/M 3.1 `A>` in 80
> columns on the Videx** — compose the apl2cpm3 board (slot 4) + the Videx (slot 3); `icrt` drives the Videx CRTC
> → `DisplayMultiplexer.ActiveIndex==1` (the **first** real-CP/M Videx engagement; closes ADR 0017 OQ2). Gate:
> the decoded CP/M-3 console on the Videx 80×24 render + `ActiveIndex==1` + the captured frame hash. **Strictly
> ordered V80-1 → V80-2 → V80-3** (the live apl2cpm3 Disk 1 is every PR's un-fakeable gate; on-screen 80-col
> `A>` is the arbiter). Coprocessor is **interpreter-tier only** (honor interpreter-as-oracle, un-fakeable gates,
> AOT-clean Core). **Drift from ADR 0018 flagged in the plans:** (1) `ControlPortBase` kept as an alias to
> `ControlPortBaseSlot5` (zero caller breakage — `SoftCardVidexBoard`/surfaces still reference the bare name);
> (2) the apl2cpm3 disk is staged at the throwaway `cpm/videx-candidates/` investigation path, but the loader
> reads the production `cpm/apl2cpm3/` path (the Builder copies Disk 1 there / runs the fetch script — honors
> "do NOT clobber the 2.2 disk" via a separate subdir); (3) V80-2's bring-up content is determined live, not
> pre-designed (ADR Decision 3); (4) the 40-col-vs-80-col staging (OQ2) is decided by the live boot — if
> apl2cpm3 goes straight to 80-col, V80-2's 40-col gate is re-scoped into V80-3. **V80-1 is Builder-ready, no
> deps, topmost.** Disks 2–7 are NOT needed (Disk 1 boots standalone — optional follow-on UAT only). **Next:
> Builder picks up V80-1** (additive, defaulted slot param + asset loader + honest skip — the 2.2 board
> untouched). Prior Builder log:
> **Last updated:** 2026-06-21 (Builder — **CPM-5 SHIPPED — the Videx CP/M gate re-frame (ADR 0017 Decision 6),
> the 80-col auto-engage question RESOLVED.** Discovery (OQ2): the Builder booted **all five** owner-downloaded
> candidate SoftCard CP/M masters (`cpm223-60k`, `ms-softcard-ii-228b`, `cpm-z80softcard`, `softcard-1980`,
> `premium-iie-225`) on the real `SoftCardVidexBoard` (Videx wired) with CP/M now reaching `A>`, instrumenting
> the Videx auto-engage signal (`videx.ActiveChanged` → `DisplayMultiplexer.SetActive`). **NONE auto-engages the
> Videx:** three share the cached master's boot1 sector but **crash in 6502 boot2** (undefined opcode `$B7` at
> `$0FAD` — a different on-disk data-track skew than the cached 44K master's `SectorOrderKind.Cpm`); two have a
> different boot loader and never hand off to the Z80. On every candidate `ActiveChanged` fired 0× and
> `ActiveIndex` stayed 0. **So per Decision 6 the gate is RE-FRAMED to the hardware truth** (the fall-back
> branch): un-skipped `Cpm_boots_and_renders_the_A_prompt_on_the_Videx_80col_interpreter` (was an empty
> `[Fact(Skip=…)]`) into a live `[SoftCardCpmFact]` — the cached 40-col master boots to `A>` on the
> SoftCard+Videx board with the production auto-switch wiring live, asserting `A>` + a CP/M sign-on +
> `CoprocessorActive` + the Videx **never engaged** (`videxEngagedCount==0` AND `mux.ActiveIndex==0` — asserting
> `ActiveIndex==1` for a 40-col master would force a fake). The Videx 80×24 render path is proven **independently
> + asset-free** by the existing PR-N `VidexVideotermTests` direct-render gates. **Pre-merge review
> (`feature-dev:code-reviewer`): 0 HIGH / 0 MEDIUM / 0 LOW** (one cosmetic comment note, fixed). Full Release
> suite **7311 passed / 0 failed / 3 skipped**, warning-clean (+1 net — the Videx `A>` gate moved skip→pass; the
> skip count dropped 4→3). **Proofs:** the legible 40-col `A>` boot (`/d/prj/cpm-A-prompt.png`); the active-source
> render on the Videx board staying Apple-40 (`/d/prj/cpm-videx-40col-active.png`); the asset-free direct Videx
> 80×24 render (`/d/prj/cpm-videx-80col.png` — structural, via the synthetic `VidexFont`; legible glyphs need the
> optional `get-videx-roms` char ROM). ADR 0017 Decision 6 + OQ2 RESOLVED in the ADR. **An 80-col CP/M master
> remains an owner-asset item** (Decision 7); if one is sourced, the gate gains a sibling asserting
> `ActiveIndex==1`. **This closes the SoftCard CP/M arc** (CPM-1…CPM-5 ✅). Backlog row W (`WozFluxImage`) +
> deferred row L (JIT-under-translation) remain. Prior Builder log:
> **Last updated:** 2026-06-21 (Builder — **CPM-5 claimed (🔨)** on `feat/apple2-cpm-5-videx-reframe` —
> the Videx CP/M gate re-frame + the 80-col auto-engage discovery (ADR 0017 Decision 6 / OQ2). Boot the 5
> owner-downloaded candidate masters on the SoftCard+Videx board; if one auto-engages the Videx, build the
> real 80-col gate; else fall back to the 40-col `ActiveIndex==0` re-frame.) Prior Builder log:
...
> not after the whole slice budget (ADR 0017 Decision 3, amends ADR 0015 D1). The old whole-slice `Run`
> corrupted every Z80↔6502 BIOS round-trip. **The change is confined to the `RunDualCpu` method body** —
> `RunSingleCpu` + the `Run` dispatcher are **byte-for-byte unchanged** (the load-bearing regression
> invariant). **Un-fakeable gates:** (1) a synthetic dual-CPU yield test (`DualCpuYieldTests`) — the 6502
> writes `$C200` then a `$5A` sentinel; with the old whole-slice `Run` the 6502 ran past its toggle and wrote
> `$5A` before yielding (verified FAIL pre-fix), with the per-instruction yield the sentinel stays unwritten
> until control returns (PASS post-fix); (2) the live CPM-3 gate
> (`Cpm_boot_runs_the_z80_bios_at_Axxx_stably_after_the_run_loop_yield`, asset-gated, runs live here) samples
> the Z80 PC across the real boot — **pre-fix the Z80 collapses ENTIRELY to its `$0000` reset stub** (100% of
> active samples `$0000-$000C`, never reaches `$Axxx`); **post-fix it runs real CP/M BIOS at `$ABxx` stably**
> (135/138 samples, **zero** reset-stub samples). No band adjustment needed — the ADR's `$Axxx` prediction was
> exact. **Pre-merge review: 0 HIGH / 0 MEDIUM / 0 LOW — no fixer** (the reviewer traced the subtle
> forward-progress risk at high clock ratios and confirmed termination: the running-total
> `_coprocessorCyclesContributed / ratio` grows monotonically to ∞, and at the real SoftCard ratio 2.0 every
> Z80 instruction advances the virtual clock by ≥2/step — no spin). **No 4th defect:** with CPM-1+2+3 the boot
> reaches AND stably holds the CP/M BIOS at `$Axxx`, exactly as ADR 0017 predicted. **No dual-CPU cycle
> assertion shifted** (the suspended 6502 is still never stepped while `_z80Active`; running-total rounds
> once). Full Release suite **7309 passed / 0 failed / 5 skipped**, warning-clean. (One pre-existing
> load-dependent UAT flake, `WebServerSmokeTests.WebSocket_streams_a_binary_FB_frame` — a demo single-CPU
> WebSocket frame-ordering race that never enters `RunDualCpu`; passed 9/9 in isolation + green on the final
> full run.) **Rebased onto PR #132** (`apple2-surface` polish, which landed mid-cycle; no conflicts — disjoint
> files). **Next: CPM-4** — the headline `A>` deliverable (decoded-text gate + the `$1010` bridge live triage;
> may be a no-op if 1–3 are the complete gating set).) Prior Builder log:
> **Last updated:** 2026-06-21 (Builder — **CPM-2 SHIPPED (PR #131).** `SoftCardControlPort.Read()` is now
> **open-bus `0x00` with NO toggle** — only `Write()` toggles the active CPU (ADR 0017 Decision 2, amends ADR
> 0015 D3; the one-effective-line production change). **Un-fakeable gate** (asset-free, port-level): a read —
> even 1000 reads — fires **0** toggles via the `ControlSpy.Calls`; a write fires exactly **1**; the
> read/write asymmetry FAILS with the old read-toggle and PASSES with the open-bus Read (verified red→green).
> The existing `Real_Z80_runs_translated_*` handoff + `A_write_flips_*` + `TryPeek_*` facts all stay green
> (none depended on a read-toggle). **Pre-merge review: 0 HIGH / 0 MEDIUM / 0 LOW — no fixer needed.** Full
> Release suite **7306 passed / 0 failed / 5 skipped**, warning-clean. **DRIFT (live-verified, in the PR
> body) — the plan's live decoded-text gate is a NAMED-SKIP, not an assertion, here:** the plan envisioned a
> `Cpm_boot_passes_the_softcard_detect_no_cant_find_message` gate asserting the `CAN'T FIND Z80 SOFTCARD` line
> disappears after the open-bus Read. **It does not** on this build — independently re-verified against the
> cached `softcard-cpm.dsk` at the 10M-cycle budget: the decoded screen is **byte-identical** between
> read-toggle and open-bus (row 19 `CAN'T FIND Z80 SOFTCARD`, row 23 monitor `*`, `CoprocessorActive=true`)
> in BOTH. The detect livelock only clears with **CPM-3's run-loop yield** (exactly what the merged CPM-1
> comment + plan §Drift 1 / Task 2c anticipate). A decoded-text gate here would be a **false pass** (cannot
> fail with the read-toggle restored), which the plan forbids — so it ships as a named `[Fact(Skip=…)]`
> deferring the decoded-text discrimination to CPM-3, with the live observation recorded in-test. The
> un-fakeable CPM-2 proof is the port-level asymmetry test. **Next: CPM-3** (`RunDualCpu` Step-based yield at
> `$CnXX`; single-CPU path byte-for-byte unchanged).) Prior Builder log:
> **Last updated:** 2026-06-21 (Coordinator — **PR #128 (Spectrum SU + interrupt-delivery fix) and PR #130 (CPM-1 skew + de-fang) BOTH MERGED → green main restored.** Each fixed the other's "unrelated" red gate. The SoftCard CP/M arc continues: **CPM-2 → CPM-3 → CPM-4** to the live `A>`.) Prior Builder log:
> **Last updated:** 2026-06-21 (Builder — **CPM-1 implemented → PR #130 (merged).**
> CPM-1's own deliverable is complete and reviewed: the **per-track CP/M skew** (ADR 0017 Decision 1 — boot
> interleave `(p×11)%16` for system tracks 0–2, data table for 3+, via the additive
> `Apple2SectorOrder.PhysicalToLogical(kind,track)` overload + `DskFluxImage` per-track resolution; **DOS/ProDOS
> unregressed**, regression-guarded) so boot2's `$0F7D` is a valid opcode (no silent BRK-to-monitor crash); and
> **both CP/M `A>` boot gates de-fanged** to an honest negative (the skew-crash BRK register dump is gone) + a
> named `[Fact(Skip=…)]` for the `A>` part (SoftCard → CPM-4, Videx → CPM-5). The 2 CP/M gates went **RED →
> honest-skip**; the skew fix is gated by an un-fakeable regression (FAILS pre-fix / PASSES post-fix). Pre-merge
> review **0 HIGH / 0 MEDIUM / 1 LOW (fixed)**. Filtered SoftCard/skew lane **51 passed / 0 failed / 2 skipped**,
> whole-solution build **warning-clean**. **DRIFT (live-verified, in PR #130 body):** the plan's literal Task-3
> predicates (`DoesNotContain('*')` / `DoesNotContain("CAN'T FIND")`) are wrong against ground truth — both the
> pre- and post-fix screens end at a monitor `*`, and `CAN'T FIND Z80 SOFTCARD` is the POST-fix signal that the
> boot ADVANCED to the detect handshake (the CPM-2 defect, not fixed here). The honest discriminator is the
> skew-crash BRK register-dump signature; ADR 0017 root-cause step 6 is the ground truth. **WHY MERGE IS HELD:**
> the **full** Release suite is **7285 passed / 2 failed / 4 skipped** — the 2 failures are **NOT CP/M and NOT
> from this PR**: `SpectrumBootTests.Rom_boots_to_the_basic_copyright_screen` (both tiers) — `BootCycles=200_000`
> is ~30× too small (real 48K boot to the © screen ≈5.9M T-states). **These fail identically on clean `main` @
> `1a8ee9e`** (verified); they surfaced only because a Spectrum `48.rom` was cached on this machine today (it was
> skip-with-note during the prior green-suite cycle). **This is exactly what queue row SU fixes** (`BootCycles`
> 200k→7M) — out of CPM-1's scope; touching `SpectrumBootTests.cs` here would pre-empt SU. **Builder STOPPED per
> protocol (red gate → pause): owner decides whether to (a) merge #130 as-is [recommended — CPM-1 is complete +
> the Spectrum red is pre-existing/unrelated] then take SU; or (b) land SU first.** The held Spectrum PR #128
> sequencing step the owner mentioned is independent of this hold.)
> (Planner — **SoftCard CP/M boot-to-`A>` fix arc PLANNED (new rows CPM-1…CPM-4)**
...
| **W** | `WozFluxImage` — a thin `.woz`-file byte parser → `IFluxImage` | ✅ (PR #143) | F | [plan](superpowers/plans/2026-06-22-woz-flux-image.md) | The missing half of "full `.woz` fidelity upfront" (the locked decision): PR-F shipped the `.woz`/LSS **read path** + the `IFluxImage` track-bitstream **seam**, but no `.woz`-**file** byte parser. `WozFluxImage` parses the **WOZ2** container (INFO/TMAP/TRKS chunks → per-track bitstreams; **WOZ1 rejected** — decision W-1) into an `IFluxImage` the controller reads identically to the shipped `SyntheticFluxImage`/`DskFluxImage` (the seam match is direct — WOZ2 TRKS is MSB-first packed bits + a `bit_count` loop length, exactly what `TrackBits`/`TrackBitLength` need). Unblocks raw `.woz` in `DiskImageFactory.FromBytes` (today `NotSupportedException`), the R library list (today `supported:false`), and S upload (today validates magic then the honest reject). **Un-fakeable gates:** asset-free — a hand-built WOZ2 round-trips track bits + `bit_count`; a wrong CRC32 / WOZ1 / non-5.25" is rejected; TMAP resolves `t*4`. Asset-gated `[WozDiskFact]` (skip-with-note absent) — a **real fetch-on-demand, never-vendored** `.woz` is parsed, its CRC32 verified on real bytes, and its track-0 bitstream read by the **live `Apple2DiskII` head** (finds the `D5 AA 96` prologue). Interpreter-tier; AOT-clean Core (lives in `Peripherals`). *(Spec: `superpowers/specs/2026-06-22-woz-flux-image-design.md`.)* |
| **D68** | A real 68000 disassembler (field-grammar-walking) | ✅ (PR #144) | — | [plan](superpowers/plans/2026-06-22-m68000-disassembler.md) | **Roadmap deferred #6.** The field-grammar 68000 carries no `InstructionDef` rows, so the generated `Disassemble` is a `???`-only stub — the `--board m68000` monitor renders every instruction as `???`. Adds a **`FieldGrammar`-gated** branch to the generator's `EmitDisassembler` + a new `EmitM68kDisassembler` that walks `Decode68k.Ops` (the same table the decoder walks): mnemonic family + size suffix (`.B`/`.W`/`.L`) + condition code (`Bcc`/`DBcc`/`Scc`) + EA operand. Operands in **extension words** render honest placeholders (`#<imm>`/`<abs>`/`<d16>`) — the 3-byte `IMonitorSupport.Disassemble` contract is **not** widened (deferred; D68-3). **No 68000 assembler** (D68-1) → the round-trip gate is a curated `(opword, expected mnemonic)` table. **Monitor-display-only: no IL, no execution-path change** — the TomHarte/decode/JIT-parity gates are untouched. **Un-fakeable gates:** the curated round-trip corpus (each row fails on a wrong family/size/cc/EA); a **decoder-agreement** sweep (every grammar-matched opword renders non-`???` — reads the shipped `Decode68k.Ops` directly); a monitor-host gate (`NOP;MOVEQ;ADD.W;RTS` render real mnemonics). **DECISION-R regression gate:** empty git-diff on the 6502/Z80/8086 generated `Disassemble`. *(Spec: `superpowers/specs/2026-06-22-m68000-disassembler-design.md`.)* |
| **B68-DOC** | Bench doc-reconciliation — close roadmap #3 truthfully (W3 shipped, W2 off-by-2 = accepted slack) | ✅ (PR #145) | — | [plan](superpowers/plans/2026-06-22-bench-doc-reconciliation.md) | **Doc-only** (closes roadmap deferred #3). Both claims in the roadmap item are stale: (a) the **W3 profiler arm shipped** in `bc68ee7` (`bench/hotop-profiler/Profiler.cs:215` calls `Run68000("W3 sieve-kernel", …)`; the `## 68000 — W3 sieve-kernel` block is in `bench/hotop-profiler/hotop-profile-results.txt`); (b) the **W2 off-by-2 is accepted coarse-cycle slack** per **DECISION T2** (the JIT charges coarse `BaseCycles + 1` fetch vs the interpreter's exact prefetch — gated within a `<= 16` tolerance in `BenchHarnessSmokeTests`, not a bug). Edits `docs/ROADMAP.md:234-237` (retag `[resolved]`) + `docs/user-guide/benchmarks.md:148-150` (reframe the caveat as resolved). **Un-fakeable gate:** a self-pinning doc-assertion test — (1) the stale phrases are gone, (2) the resolution + citations (`bc68ee7`, `DECISION T2`) are present, (3) the cited evidence still exists in source (so the docs can't silently re-rot). Tiny. *(Spec: `superpowers/specs/2026-06-22-bench-doc-reconciliation-design.md`.)* |
| **FF-1** | The linear `(CS<<4)+IP` block key + the `IJitTarget.ProjectBlockKey` seam (NO far emit) | ✅ (PR #146) | — | [plan](superpowers/plans/2026-06-22-ff1-linear-block-key-and-projectblockkey-seam.md) | **ADR 0019 FF-1 (the SAFE-gated cache-key widening — Decision 1 + 2; the #1 owner-prioritized deferred item).** Widens the shared JIT block-cache key `ushort → uint` to the generic linear `(CS<<4)+IP` across `BlockCache`/`ChainTable`/`CompiledBlock`/`JittedCpu`/`BlockCompiler` (the ADR §3 Decision 2 blast-radius table; `_blocksByPage` page-index + the dirty-page/SMC machinery are physical-page-based and **untouched**). Adds the additive `uint IJitTarget.ProjectBlockKey(ICpuCore cpu)` — generated per-CPU by `CpuEmitter` (8086 → `((CS<<4)+IP)&0xFFFFF` when the spec declares a `CS` register; the non-segmented **6502/Z80/68000 → identity `(uint)PC`**). Folds the **existing 8086 near-flow** static chain target through the baked code-phys base (Decision 2 note); replaces the dispatcher's `ushort pc` read with `uint key = _target.ProjectBlockKey(_inner)`. **No far op is emitted in FF-1** (`FallbackEmitCount` for the far opcodes is unchanged). **Un-fakeable gate (merge precondition):** the **key-projection identity regression** — the non-segmented CPUs' block counts, chains, `TotalEvictions`/`TotalRecompiles`, SMC marks, and `FallbackEmitCount` are **byte-for-byte unchanged** vs the captured pre-FF-1 baselines (the ADR's SAFE proof), PLUS the `ProjectBlockKey == (uint)PC` unit test per non-segmented CPU + the **overlapping-segment coherence** check (two `(CS,IP)` pairs folding to the same physical → the same key) + the existing **8086 near-flow** parity stays green + TomHarte-through-JIT parity stays green for ALL four CPUs. Interpreter-as-oracle; AOT-clean Core (the projection reads via `ICpuCore.GetRegister`, no concrete-CPU dependency). **Why first:** the far arms (FF-2) are *unsound* until the key is widened; shipping the widening alone — proven SAFE by the identity gate — de-risks the far emit into a pure additive PR. *(Spec: ADR 0019 `architecture/0019-8086-far-flow-emit-and-the-cs-ip-block-key.md`.)* |
| **FF-2** | The far `JMP`/`CALL`/`RET` emit arms + the aliasing regression | ✅ (PR #147) | **FF-1** ✅ (landed + identity gate green @ PR #146 — sequencing precondition met; SEPARATE PR, NOT co-merged) | [plan](superpowers/plans/2026-06-22-ff2-far-flow-emit-arms.md) | **ADR 0019 FF-2 (the unblock — Decision 3 + 4).** Emits the regular far transfers (ADR §4 emit shapes): far `CALL` (`9A` direct chainable / `FF /3` indirect dynamic), far `JMP` (`EA` direct chainable / `FF /5` indirect dynamic), far `RET` (`CB` / `CA`-imm16, dynamic). New machinery = `EmitM8086SetCs`/`EmitM8086SetCsFromStack` (the CS-write helpers, mirroring `EmitM8086SetIp`) + the doubled far push/pop (`EmitM8086PushWord` ×2 = push CS then IP; `EmitM8086PopWord` ×2 = pop IP then CS) — the far frame per ADR 0005; the direct forms' constant `(newCS,newIP)` chain to the projected key `((newCS<<4)+newIP)&0xFFFFF` (the FF-1 payoff). Un-forces the `CpuEmitter` far-flow descriptor gate (`IsEmittableX86FarFlow`; ADR 0011 drift #1, far family populated-but-forced-fallback). **`INT`/`INTO`/`IRET`/`BOUND` stay fallback** (ADR 0011 §2/OQ5; Decision 3). **Un-fakeable gates (merge precondition):** (1) the **far-transfer aliasing regression** — two segments with the SAME offset compile to **DISTINCT** blocks (FAILS on the old `(IP)` key, PASSES on the linear key; observable via a segment-unique side-effect byte) — the load-bearing proof the widening is real and armed by an *emitted* CS-changing op; (2) **far-flow TomHarte-through-JIT byte-identical** parity for `9A`/`EA`/`CB`/`CA`/`FF /3`/`FF /5` (registers + FLAGS + CS:IP + the far stack frame + memory + cycles), `FallbackEmitCount` drops by exactly those opcodes; (3) a measured far-call-bearing throughput delta vs the frozen M6 constants (honesty gate). **Splittable:** FF-2a (direct `9A`/`EA`/`CB`/`CA` + the aliasing gate) then FF-2b (indirect `FF /3`/`FF /5`) if the far-indirect EA-resolution proves heavy — the fallback valve keeps a partial far family correct. **⛔ Sequencing (ADR 0019 §5): FF-2 depends on FF-1 and MUST NOT be merged in the same PR** — the aliasing bug only arms once an *emitted* op changes CS mid-chain, so FF-1 (the key) must land + pass its SAFE identity gate before FF-2 (the far arms) is sound. *(Spec: ADR 0019 `architecture/0019-8086-far-flow-emit-and-the-cs-ip-block-key.md`.)* |
| **SU** | ZX Spectrum 48K ROM UAT — multi-variant boot (variant × tier) + interactive BASIC | ✅ (PR #128) | — | [plan](superpowers/plans/2026-06-21-spectrum-48k-rom-uat.md) | **Recalibrates** the shipped Spectrum boot gate (`BootCycles` 200k→**7M** — full boot to the (C) screen is ≈5.9M T-states, stable by ~13M; the 200k was ~30× too small + the "≈140k/two frames" comment is wrong) and **parameterizes** it across the owner's six 48K ROM variants (canonical `spec48` + arabic-v1/v2/v31 + beckman + prototype, each 16384 B) **× both tiers**: every present variant boots to its copyright screen on Interpreter AND Jit — structural assertion (mostly-white `Colors[7]`=`0xFFD7D7D7` paper + black `Colors[0]`=`0xFF000000` ink, variant-safe floor `>50`, canonical `>200`) + a per-variant committed RGBA hash (captured on first green; both tiers identical). Adds a `<cache>/spectrum/variants/<name>.rom` discovery helper (`SpectrumRomVariants.Discover`, skip-with-note when none) + `get-spectrum-rom-variants.{sh,ps1}` (owner-copy, never vendored). **Plus an interactive BASIC UAT** (canonical ROM, both tiers): boot to the `K` cursor, drive the key matrix to type `PRINT 2+2`+ENTER (keyword `P`→`PRINT`, `SymbolShift`+`K`=`+`, `Enter`) and assert the printed `4` appears in the top print rows (ink-delta + committed hash) — boot → keyboard → BASIC interpreter → screen end-to-end. **Asset-gated** (skip-with-note when no Spectrum ROM cached). 48K-only (128/+2/+3 are a separate future arc). Flags (not built): a `--board` surface override + the Tester scratch cleanup (`tools/SpectrumProbe/`, `tools/WsProbe/`, `.uat-artifacts/`). |
...
| **CPM-4** | The live `A>` deliverable: decoded-text gate (Decision 5) + `$1010` bridge bring-up (Decision 4) | ✅ (PR #134) | CPM-3 | [plan](superpowers/plans/2026-06-21-cpm-boot-to-a-prompt-pr4.md) | **THE HEADLINE.** With CPM-1–3 landed, brings the boot to `A>`; closes any residual `$1010` 6502-BIOS↔Z80 bridge item via **live triage against the real disk** (Decision 4 — Builder bring-up, may be a no-op if 1–3 are the complete gating set; not pre-designed). **Un-fakeable gate (Decision 5):** decode the 40-col Apple text page (`TextRowBase` walk, high-bit-stripped) and assert the **decoded `A>` substring** (the CCP prompt) + a CP/M sign-on line (`CP/M`/`DIGITAL RESEARCH`) + `CoprocessorActive==true`; **capture the real frame hash** (replace `PLACEHOLDER` in the SAME PR — the text substring is the primary oracle, the hash a tightening gate). **Asset-gated** (interpreter tier; the coprocessor has no JIT). Owner UAT = the visible `A>` in the browser surface. **OUTCOME: outcome (1) — fixes 1-3 were the complete gating set; the `$1010` bridge was a no-op; test-only.** |
| **CPM-5** | Videx CP/M gate re-frame (ADR 0017 Decision 6) — 40-col `ActiveIndex==0` master + the asset-free direct Videx render gate; 80-col auto-engage discovery (OQ2) | ✅ | CPM-4 | discovery (ADR 0017 D6/D7) | **ADR 0017 Decision 6.** First: **discovery (OQ2)** — boot the 5 owner-downloaded candidate masters (`cpm223-60k`, `ms-softcard-ii-228b`, `cpm-z80softcard`, `softcard-1980`, `premium-iie-225`) on the SoftCard + Videx board (Videx slot) and detect Videx auto-engagement (`videx.ActiveChanged` fired / `DisplayMultiplexer.ActiveIndex==1` / `$C0Bx` accesses). **If a master auto-engages:** build the real 80-col end-to-end gate + wire it as the 80-col asset. **If none does (the re-frame):** un-skip `Cpm_boots_and_renders_the_A_prompt_on_the_Videx_80col_interpreter` as the honest 40-col gate — the cached master boots to `A>` on the SoftCard+Videx board with `ActiveIndex==0` (multiplexer stays Apple-40 — the hardware truth for a 40-col master), `CoprocessorActive`, Videx never engaged; the asset-free **direct Videx 80×24 render** is already proven by the PR-N `VidexVideotermTests`. **Un-fakeable gate:** the live 40-col `A>` on the Videx board with `ActiveIndex==0` (asset-gated) + the existing direct-render gates (asset-free). Document which masters were tested + the finding in ADR 0017 D6. |
| **V80-1** | Configurable SoftCard slot (per-board `controlPortBase`) + the `Apl2Cpm3` asset loader + an honest skipped boot gate | ✅ (PR #137) | — | [plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr1.md) | **ADR 0018 Decision 1 + 5** (apl2cpm3 CP/M 3.1 / Videx-80-col sub-arc). Makes the SoftCard control-port slot a **per-board parameter** (`SoftCardBoard.Spec`/`SoftCardVidexBoard.Spec` gain an optional `controlPortBase`, **defaulted to slot 5 / `$C500` so the 2.2 board is byte-for-byte unchanged**; apl2cpm3 passes slot 4 / `$C400` — the gating fix: apl2cpm3 hard-codes `STA $C400` [live: `8D 00 C4` at track-0 `$DBA`], at slot 5 it prints `NO Z80 FOUND`, at slot 4 the Z80 activates + CPMLDR loads to `$1100+`). Adds `Apl2Cpm3` asset loader (distinct `cpm/apl2cpm3/CPM3.1_Disk_1.dsk` cache path — never clobbers the 2.2 `cpm/softcard-cpm.dsk`) + `tools/get-apl2cpm3.{sh,ps1}` (fetch-on-demand, sign-off **covered by the existing CP/M-disk sign-off**). The apl2cpm3 40-col boot gate is **present but named-skipped** until V80-2. **Un-fakeable gate** (asset-free): a slot-placement run-loop test — a `$C400` write toggles the slot-4 SoftCard's coprocessor + a `$C500` write does not (and the mirror for the default board), all four `[Theory]` rows green; the slot-4 board validates+builds (`$C400` contained in the `$C000-$C5FF` Mmio region); the **full 2.2 suite green** (the CPM-4 hash gate + CPM-5 Videx gate pass unchanged). PR-1 alone does **not** reach `A>`. |
| **V80-2** | CP/M 3.1 reaches `A>` (the track skew — `SectorOrderKind.Cpm3` raw-DOS33; the `?jsr65` bridge confirmed already working, no plumbing) | ✅ (PR #139) | V80-1 | [plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr2.md) | **ADR 0018-A + 0018-B** — THE 40-col milestone; **THREE layers, two fixes in one PR.** **Layer 1 (Z80-reset escalation): VOID** (0018-A — the NOP-slide-to-`$0100` cold-start is faithful; **no Z80-core change**). **Layer 2 (skew, SOLVED live):** present apl2cpm3's boot **track 0 in raw DOS 3.3 order** (`Dos33PhysToLog`; tracks 1–2 keep `CpmBootPhysToLog`, `table[0]==0`) so BOOTLDR's own `xlt` deskews CPMLDR — `$31` lands at Z80 `$0100`, CPMLDR loads + runs forward, opens the `"CPM3 SYS"` FCB (land as a new `SectorOrderKind.Cpm3`; 2.2 `Cpm` untouched). **Layer 3 (the gating bridge — 0018-B):** CP/M-3's LDRBIOS does ALL I/O (the `CPM3.SYS` floppy read via `?fdrwts`, the console via `?odcrt`) by calling back into the 6502 via `?jsr65` — it toggles the bus to the 6502 and waits for the 6502's `L65A` service loop ($03C0) to run the sub + `STA $C400` the bus back. Our handoff flips a boolean with **no PC redirect**, so the 6502 resumes at BOOTLDR's stale `STA $C400 / RTS` overlay ($03C9), never enters `L65A`, runs neither `?fdrwts` nor `?odcrt` → the `CPM3.SYS` read fails (`A=$FF`, Z80 `DI/HALT`s at `$01A9`) AND zero console output, from ONE dead bridge (live: a single `6502→Z80`, zero hand-backs). **Fix:** an additive, per-board **"6502 resume entry on bus-hand-to-6502"** on the `ICoprocessorControl`/`SoftCardBoard.Spec` handoff seam (ADR 0017 D3 territory), **defaulted off so the 2.2 board is byte-for-byte unchanged**; apl2cpm3 passes the LDRBIOS service entry ($03C0/$03C6/$03D0 — pinned live). **Still no Z80-core change, no owner sign-off** (0018-A/B). **Un-fakeable gate** (`[Apl2Cpm3Fact]`, asset-gated): the live apl2cpm3 Disk 1 boots to **CP/M 3.1 `A>`** on the 40-col `$0400` page — decoded `A>` + a CP/M-3 sign-on substring (`CP/M V3.0`/`DIGITAL RESEARCH`) + `CoprocessorActive`; plus the fast discriminators — `$31` at Z80 `$0100` (skew) + **≥1 `Z80→6502` hand-back** (bridge; 0 today). Capture the `A>` PNG via `BootProbe --cpm-screenshot --apl2cpm3`. |
| **V80-3** | THE HEADLINE: CP/M 3.1 `A>` in 80 columns on the Videx — `ActiveIndex==1` from a real CP/M boot | ✅ (PR #139) | V80-2, V80-4 | [plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr3.md) | **ADR 0018 Decision 4 — THE DELIVERABLE, SHIPPED** (the true 80-col CP/M end-to-end the 2.2 arc could not produce; closes ADR 0017 OQ2/D6's `ActiveIndex==1`-from-real-CP/M sibling-gate). **Live pin (ADR 0018-C OQ1):** apl2cpm3 programs the Videx CRTC (`$C0B1` data writes: 420) but NEVER does a `$C0B8-$C0BF` bank-select (0), so the old bank-select-only `ActiveChanged` trigger never fired (`ActiveIndex` stayed 0). **Fix:** `VidexVideoterm.WriteReg` now ALSO engages the Videx on a **CRTC-data write** (`$C0B1` — the firmware bringing the 80-col display online); the `SetActive` no-op guard fires `ActiveChanged(true)` exactly once, so `DisplayMultiplexer.ActiveIndex` flips to 1 from the real boot. The 2.2 master issues ZERO `$C0Bx`, so it never engages — the CPM-5 `ActiveIndex==0` gate is unchanged (contrast sibling; do NOT modify it). **Un-fakeable gate** (`[Apl2Cpm3VidexFact]`, asset-gated): the live apl2cpm3 Disk 1 renders **CP/M 3.1 `A>` in 80 columns on the Videx** with `videxEngagedCount>0` + `mux.ActiveIndex==1` + `CoprocessorActive` (sampled across the boot) + the decoded `A>` off the `$CC00` VRAM. Screenshot `/d/prj/cpm-videx-80col-A-LIVE.png`, frame SHA-256 `627a1657…5004`. |
| **V80-4** | The Language-Card two-latch write-enable correction (74LS175) → the fifth-layer fix that lets CP/M 3.1 reach `A>` | ✅ (PR #139) | V80-2 | [ADR 0018-C](architecture/0018-C-apl2cpm3-language-card-bank2-write-enable-flip-flop.md) | **ADR 0018-C — classified SAFE, live-proven.** `src/CpuEmulator.Peripherals/Apple2LanguageCard.cs` `Access` now models the real 74LS175 **two latches** (MAME `ramcard16k` `do_io` + Sather ch.5): an **even** access clears both the pre-write count AND write-enable; an **odd-address WRITE** clears only the count (write-enable, if armed, SURVIVES — the bug fix: the old single-flag model write-protected LC bank 2 on apl2cpm3's `LD ($C08B),A` bank-2 select, dropping the `?ldccp` `LDIR` CCP copy → bank 2 zeroed → `RET` into a zeroed `$1901`); an **odd-address READ** arms/enables (two consecutive). Single method, no signature/seam change. **NO** Z80-core / `SoftCardTranslation` / `Machine`/`ICoprocessorControl` / `SectorOrderKind` / 2.2-board change. **Gates:** `[Apl2Cpm3VidexFact]` strengthened to the decoded `A>` off the `$CC00` VRAM; a **LC-bank-2-nonzero discriminator** (3026/4096 post-LDIR, 0 under the old model); a **positive LC unit test** (odd write preserves write-enable — FAILS on the old model, PASSES on the fix). **Regression guard — all GREEN live:** the 2.2 CPM-4 (`A>` committed hash) + CPM-5 (Videx `ActiveIndex==0`) gates, all 13 LC unit tests, the full Apple2/SoftCard/dual-CPU sweep. |
| **MD** | 8086 MUL/DIV emit (`F6`/`F7` /4../7 — MUL/IMUL/DIV/IDIV) | ✅ (PR #149) | — | [plan](superpowers/plans/2026-06-23-8086-muldiv-emit.md) | **ROADMAP #4 (the finale), priority 1 of 3.** The microcoded multiply/divide — `F6`/`F7` /4 MUL /5 IMUL /6 DIV /7 IDIV (byte+word) — now EMITS (the new `EmitM8086MulDiv` arm), transcribing `AluMul`/`AluDiv` (`M8086Cpu.Alu.cs:369/412`) one-for-one. The ALU arm already emits `F6`/`F7` /0../3; this fills the /4../7 hole the `IsEmittableX86Family` ALU-mnemonic whitelist currently leaves fallback. **MUL/IMUL straight-line** (set CF=OF on upper-half significance; SF/ZF/PF/AF left as the interpreter's natural fallout); **DIV/IDIV fault-capable** — divide-by-zero OR quotient-overflow (incl. the 8086 IDIV symmetric-range quirk `|quot|>127`/`>32767`) vectors INT0 via the new shared `EmitM8086RaiseInterrupt` IVT-push helper (DECISION MD-2, **owned here**, reused by row II), so **DIV/IDIV END the block** (DECISION MD-1, endsBlock re-forced like the flow rows; MUL/IMUL do not). **Honors ADR 0011 §2/§5/OQ5** — the interpreter is the oracle + byte-exact fallback; emit is a pure perf dial. **Stays fallback by design (noted, not forced):** BOUND, AAM/AAD (out of #4 scope), and the divide-error UNDEFINED arithmetic-flag fallout (DD6) — emit reproduces the interpreter's *modeled* behavior exactly, so the existing divide-error + IDIV-sign-quirk corpus classifiers keep deferring the same cases. **Un-fakeable gate:** (1) **parity** — the `F6`/`F7` files in the `M8088JitTom` sweep byte-identical through the JIT to the interpreter, **now via emitted IL** (was fallback; the DD6/IDIV-quirk classifiers still defer the same cases because emit == interpreter); (2) **emits-not-fallback discriminator** — a new `M8086MulDivEmitSelections > 0` over an `F6`/`F7` /4../7 block AND `FallbackEmitCount == 0` for a MUL block + a non-faulting DIV block, proven red→green (before the gate-flip: count 0 / Fallback 1; after: count > 0 / Fallback 0). Plus a focused `M8086MulDivEmitTests` (CF/OF significance pockets, the IDIV symmetric-range boundary, the divide-by-zero INT0 frame byte-identical to a fresh interpreter). AOT-clean Core (all IL in `CpuEmulator.Jit`; one generator gate-flip; interpreter oracle unchanged). *(Spec: `superpowers/specs/2026-06-23-8086-muldiv-string-int-emit-design.md`.)* |
| **STR** | 8086 string/REP emit (`A4`-`A7`/`AA`-`AF` MOVS/CMPS/STOS/LODS/SCAS + REP/REPE/REPNE) | ✅ (PR #150) | — | [plan](superpowers/plans/2026-06-23-8086-string-rep-emit.md) | **ROADMAP #4, priority 2 of 3.** The CX-counted, DF-directed string loops — MOVS (`A4`/`A5`), CMPS (`A6`/`A7`), STOS (`AA`/`AB`), LODS (`AC`/`AD`), SCAS (`AE`/`AF`), byte+word, REP-prefixed or not — now EMIT (the new `EmitM8086String` arm), transcribing `StringExecute` + `StringStep` (`M8086Cpu.String.cs:38/29`) one-for-one. Source = `DS:SI` (DS override-replaced via the captured prefix); dest = `ES:DI` (**ES non-overridable**). The REP path emits the **one genuinely-new IL shape in #4** — a runtime in-op back-edge `while (CX!=0){CX--; body; if(isCompare && zf!=repWhileZfSet) break;}` (the Z80 LDIR precedent), with the REPE(`F3`)/REPNE(`F2`) ZF early-exit + the CX=0 zero-iteration case. **String ops do NOT change CS and the loop terminates in-op → straight-line** (DECISION STR-1, no endsBlock re-force); the loop body re-resolves the EA from the live SI/DI each iteration (DECISION STR-2). **Honors ADR 0011 §2/§5.** **Un-fakeable gate:** (1) **parity** — the `A4`-`A7`/`AA`-`AF` files in `M8088JitTom` byte-identical through the JIT (incl. the REP-prefixed cases), now via emitted IL; (2) **emits-not-fallback discriminator** — a new `M8086StringEmitSelections > 0` AND `FallbackEmitCount == 0` for a REP MOVS block + a REPE CMPS block, proven red→green. Plus `M8086StringEmitTests` (both DF directions; REPE CMPSB early-exit at the exact CX/SI/DI; REPNE SCASW; CX=0 no-op). AOT-clean Core. *(Spec: same as MD.)* |
| **II** | 8086 INT/INTO/IRET emit (`CD`/`CC`/`CE`/`CF`) | ✅ (PR #151) | — | [plan](superpowers/plans/2026-06-23-8086-int-iret-emit.md) | **ROADMAP #4, priority 3 of 3 (the autonomous run ends after this merges).** The soft-interrupt vectoring — `CD` INT imm8, `CC` INT3 (vector 3), `CE` INTO (vector 4 iff OF set), `CF` IRET — now EMITS (the new `EmitM8086Interrupt` arm), transcribing `InterruptExecute` + `RaiseInterrupt` (`M8086Cpu.Interrupt.cs:54/39`) one-for-one. INT n/INT3 advance IP to the return point THEN raise via the **shared `EmitM8086RaiseInterrupt`** (introduced by row MD — DECISION MD-2; **soft coupling**: if MD has merged the helper exists, else row II introduces it via its Task 0 — symmetric, additive); INTO branches on OF; IRET pops IP:CS:FLAGS with the 8086 reserved-bit forcing (`(popped & 0x0FD5) | 0xF002` — the corpus `0x28CF→0xF8C7` case). **Every form changes CS:IP → ENDS the block** (DECISION II-1, endsBlock re-forced). **Stays fallback by design (noted):** BOUND (`62`/`63`) — out of #4 scope. **Honors ADR 0011 §2/§5 + ADR 0005** (the IVT frame + the FLAGS forcing). **Un-fakeable gate:** (1) **parity** — the `CD`/`CC`/`CE`/`CF` files in `M8088JitTom` byte-identical through the JIT (the pushed frame in stack RAM + the vectored CS:IP + IF/TF clear + IRET's reserved-bit forcing), now via emitted IL; (2) **emits-not-fallback discriminator** — a new `M8086InterruptEmitSelections > 0` AND `FallbackEmitCount == 0` for an INT block + an IRET block, proven red→green. Plus `M8086InterruptEmitTests` (the full frame + vector; INT3; INTO taken/not-taken; IRET forcing) + a **shared-helper frame-identity cross-check** vs row MD's divide-error (proving both call the same `EmitM8086RaiseInterrupt`). AOT-clean Core. *(Spec: same as MD.)* |

---

## ~~Blocker~~ RESOLVED — V80-2 / V80-3 (the apl2cpm3 `A>` fifth layer): the Language-Card bank-2 write-protect drops the CP/M-3 CCP copy

> **✅ RESOLVED 2026-06-22 by V80-4 (ADR 0018-C, PR #139).** The Architect root-caused this to the Language-Card
> write-enable flip-flop conflating the real 74LS175's TWO latches and clearing write-enable on an odd-address
> WRITE (classified **SAFE** — the fix makes the LC model MORE faithful to documented hardware), and the Builder
> productionized it: `Apple2LanguageCard.Access` now preserves write-enable across the odd-address bank-2-select
> write, so apl2cpm3's `?ldccp` `LDIR` CCP copy lands (LC bank 2 = 3026/4096 nonzero, was 0) and the boot reaches
> `A>` in 80 columns on the Videx (`ActiveIndex==1` via the V80-3 CRTC-program auto-engage). The 2.2 CPM-4/CPM-5
> gates + all 12 prior LC unit tests stayed green live (no DOS/Applesoft/2.2 regression). Original filing below for
> the record.
>
> **Filed by Builder 2026-06-22 (PR for the V80-2/V80-3 sign-on milestone — see the banner).** V80-2 + V80-3 were
> ⛔ blocked on a single, precisely-pinned defect. Three of the planned layers are cleared and shipped (the
> `SectorOrderKind.Cpm3` raw-DOS33 skew; the `?jsr65` bridge confirmed already working — **no plumbing**; the real
> Videx firmware drives the 80-col console + paints the genuine CP/M-3 sign-on). The boot then stalls **before
> `A>`** on a fifth layer that needs a **risky change to shared shipped code** — outside V80-2's apl2cpm3-scoped
> seams and the off-limits set ADR 0018-A A1 named — so per protocol Builder STOPPED and files it here.

**Symptom (deterministic, reproduced byte-identically):** after the CP/M-3 sign-on renders on the Videx, the CCP
runs (`JP` Z80 `$0100`, `CALL 5` = BDOS); the banked BDOS computes a pointer and `RET`s to Z80 `$1901`
(physical `$2901`), which is **all zeros and was NEVER written during the boot** — the Z80 NOP-slides forever (no
`A>`). Pinned at instr 36583, `PC=$1929`.

**Root cause (diagnostic-classified BUCKET A — a load/banking defect, NOT a Z80-core or translation
mis-execution; the Z80 executes correct code on correctly-loaded inputs and computes `$1901` correctly):** the
CP/M-3 loader's `?ldccp` (apl2cpm3 `BOOT.MAC`) copies the CCP into **Language-Card bank 2** with
`LD ($C08B),A` (select extra/bank-2) → `LDIR` (`$0100`→`$B000`) → `LD ($C083),A` (back to TPA/bank-1). Our
`src/CpuEmulator.Peripherals/Apple2LanguageCard.cs` (`Access`, the `$C08x` write-enable/bank-select truth table)
treats those **bank-select writes** as forcing `_writeEnabled=false`, so `ApplyMapping` remaps bank 2
`writable:false` and the `LDIR` stores are **silently dropped** — **LC bank 2 stays zeroed** (probe: `_bankD2`
0/4096 nonzero despite 1015 `$C08x` accesses). Every later banked operation that references that region (the
`$1901` pointer chain) lands on zeros.

**Why it's a Blocker, not in-cycle bring-up:** the fix is to the **shared Language-Card write-enable flip-flop
model** — used by DOS 3.3, Applesoft, the base ][+ boot, AND CP/M 2.2 (PR-E, the first `Remap` consumer). It is
NOT the apl2cpm3-scoped `DskFluxImage`-skew / asset seam V80-2 was scoped to, and ADR 0018-A A1 + the V80-2 hard
constraints explicitly put the LC-banking model off-limits for this PR. Changing it risks regressing the base
machine + 2.2. That is exactly the "risky change the ADR ruled out" Builder must escalate.

**Suggested fix direction (for the Architect to scope, likely an ADR 0018-C + a new row V80-4):** verify the real
Apple Language Card write-enable F/F behavior under a soft-switch **write** (`STA $C08x` / `LD (nn),A`) — the
hardware likely arms write-enable across the bank-select sequence the loader relies on (or the bank-select write
should not clobber `_writeEnabled`). Whatever the corrected truth table, it MUST keep DOS 3.3 / Applesoft / the
base boot / CP/M-2.2 byte-for-byte (regression-gated — the existing LC tests + the CPM-4/CPM-5 gates are the
guard). Once LC bank 2 is populated, re-run the live apl2cpm3 boot: the BDOS pointer chain should resolve and the
boot should advance toward `A>` (then re-confirm the V80-3 `ActiveIndex==1` Videx auto-switch — apl2cpm3 paints
VRAM linearly at `$CC00`/`$CD00` and never bank-switches via `$C0B8-$C0BF` in the sign-on window, so the current
`ActiveChanged` trigger does not fire; whether `icrt`'s CRTC program should drive engagement is a V80-3-time
question once `A>` lands). **Do NOT widen scope speculatively** — the LC bank-2 write-enable is the one pinned
defect; close it, then let the live `A>` reveal any tail (ADR 0018-B §8 already flags a possible data-track skew
re-verify, now testable with the bridge live).
...
- **The 8086 far-flow arc (FF-1 → FF-2)** is the TDD decomposition of ADR 0019 (the #1 owner-prioritized
  deferred item — the most-named M6 gap; it unblocks real-mode 8086 programs through the JIT). **FF-1**
  (no deps) widens the shared JIT block-cache key `ushort → uint` to the linear `(CS<<4)+IP` + adds the
  per-CPU `IJitTarget.ProjectBlockKey` projection (the non-segmented CPUs project the identity `(uint)PC`,
  so they are byte-for-byte unchanged — the SAFE proof, gated by the key-projection identity regression);
  it emits **no** far op. **FF-2** (dep FF-1) emits the far `JMP`/`CALL`/`RET` arms (CS-load + doubled far
  push/pop), gated by the aliasing regression (two segments / same offset → distinct blocks). **⛔ FF-2 MUST
  NOT be co-merged with FF-1** (ADR 0019 §5): the aliasing bug only arms once an *emitted* op changes CS
  mid-chain, so FF-1 (the key) must land + pass its SAFE identity gate before FF-2 (the far arms) is sound.
  FF-1's `CpuEmitter` edit is the additive `ProjectBlockKey` generation; FF-2's is the far-family
  gate-un-force — no collision with any other deferred item. **Builder clears these last: W → D68 → B68-DOC
  → FF-1 → FF-2.**
