...
> **Last updated:** 2026-06-21 (Builder — **CPM-3 claimed (🔨)** on `feat/apple2-cpm-3-rundualcpu-step-yield`
> from green main @ `ae6d831`. `RunDualCpu` → Step-based per-instruction yield at the `$CnXX` write;
> `RunSingleCpu` byte-for-byte unchanged.) Prior Builder log:
