...
> **Last updated:** 2026-06-21 (Builder — **SU SHIPPED (PR #PENDING) — the ZX Spectrum 48K ROM UAT + a real
> interrupt-delivery fix it surfaced.** All **six variants × both tiers** boot-green (beckman at 22M for its
> coloured-block reset; the other five at 7M) with committed per-variant RGBA hashes (spec48 + prototype share a
> pixel-identical (C) frame, so they share a hash by design); the interactive `PRINT 2+2`→`4` UAT passes both
> tiers (the `4` at row 0, `0 OK, 0:1` at row 23 — font-verified). **The live UAT surfaced + fixed a genuine
> production bug** (DECISION 1): `SpectrumUla.OnFrameTick` asserted AND released `/INT` in one synchronous
> callback, so the Z80's level-sampled IM1 was never serviced (the 50 Hz keyboard/`FRAMES` ISR never ran); the
> ULA now holds `/INT` ~32 T-states via a scheduled release (`ScheduleAt(CurrentCycle + 32, …)`), with a
> regression gate asserting `FRAMES` ($5C78) increments over N frames. DECISION 2: per-variant boot budget
> (beckman 22M). **Pre-merge review: 0 CRITICAL / 0 HIGH** ("the scheduled-release model is correct and safe to
> merge into shared Peripherals code"); M1 fixed (test-helper path delegated to `SpectrumRom.TryGetPath`), M2
> noted (the shared spec48/prototype hash — acceptable, structural floor still gates). Scratch cleaned
> (`SpectrumProbe` kept + added to the `.slnx`; `WsProbe`/`.uat-artifacts/` removed). **Full suite: 7300 passed /
> 2 failed / 2 skipped** — the **2 failures are PRE-EXISTING + unrelated** (`SoftCardBoardTests` /
> `SoftCardVidexBoardTests` CP/M Z80 bus-master handoff; they fail identically on `main`, no Spectrum reference,
> and are the separate in-flight CP/M-bug Builder's territory). All 56 Spectrum tests green, warning-clean.)
