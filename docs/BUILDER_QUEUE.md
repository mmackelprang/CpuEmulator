...
| **V80-2** | CP/M 3.1 reaches `A>` on the 40-col console (close the Z80-entry handoff via live triage) | 📋 | V80-1 | [plan](superpowers/plans/2026-06-22-apl2cpm3-80col-pr2.md) | **ADR 0018 Decision 3 + 4** — THE 40-col milestone. With V80-1's slot fix the Z80 activates + the CPMLDR loads, but the Z80 NOP-slides from its `$0000` reset PC because the **entry vector to the loaded CPMLDR is absent**. Close it by **live triage against the running apl2cpm3 disk** (the ADR 0017 Decision 4 / `$1010`-bridge discipline — instruction-step the boot, confirm/falsify the ordered hypotheses, land the minimal change; most likely **outcome (1): no production change**, the slot fix + the shipped per-instruction yield + slot-4 handshake suffice, like CPM-4). Reuse the existing `SectorOrderKind.Cpm` skew (Decision 2 — no new skew). **⚠ FLAGGED RISK** (ADR 0018 D6/OQ1): if the bring-up shows apl2cpm3 needs the SoftCard to latch a **non-`$0000` Z80 start PC** (rather than the loader writing a `JP` stub), that touches the Z80-core reset path — judged unlikely; **the Builder PAUSES + reports** (no Z80-core change without owner sign-off). **Un-fakeable gate** (`[Apl2Cpm3Fact]`, asset-gated): the live apl2cpm3 Disk 1 boots to **CP/M 3.1 `A>`** on the 40-col `$0400` page — decoded `A>` (CCP prompt) + a CP/M-3 sign-on substring (`CP/M V3.0`/`DIGITAL RESEARCH`) + `CoprocessorActive`; the gate FAILS on the slot-5 board / the NOP-slide, PASSES only on a real slot-4 boot. Capture the `A>` PNG via `BootProbe --cpm-screenshot --apl2cpm3`. |
