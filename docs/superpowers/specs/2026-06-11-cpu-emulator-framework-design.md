...
- **M1 — the spine**
  ...
  5. Peripherals (Ram/Rom/SimpleUart) + Host + Klaus demo running.
- **M2 — the JIT**
  6. Block compiler + cache + fastmem split.
  7. Parity harness + benchmarks.
- **M3 — the pluggability proof**
  8. Z80 by spec only (prefix opcodes, separate I/O space, R refresh register). Framework changes
