...
- **M1 — the spine**
  ...
  3. Full documented-opcode 6502 spec, TomHarte green.
  4. Peripherals (Ram/Rom/SimpleUart) + Host + Klaus demo running.
- **M2 — the JIT**
  5. Block compiler + cache + fastmem split.
  6. Parity harness + benchmarks.
- **M3 — the pluggability proof**
  7. Z80 by spec only (prefix opcodes, separate I/O space, R refresh register). Framework changes
...
- **M4+ (phase-A horizon):** 8086 or 68000, timers + interrupt controller, real board
  recreations, NuGet packaging polish.
