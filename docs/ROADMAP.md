...
| **Performance feedback loop — first move (ADR 0022 items A + C, 2026-06-23)** | The standing measurement engine that turns "interesting metrics" into "here's what to speed up first." **C — free JIT counters:** four additive, zero-cost run-lifetime counters on the `IJitMetrics` seam — `ChainEdgesTaken` / `DispatcherEntries` (the rediscovered ADR-0012 floor signal: in-frame chain hops vs dispatcher round-trips) + `BlockCacheHits` / `BlockCacheMisses` — incremented on paths that already run (no per-instruction cost; the interpreter cores don't implement `IJitMetrics`, so Core stays AOT-clean; the perf-overlay wire is untouched — these are profiler-only per ADR §5). Gated by a red→green test (`ChainDispatchCountersTests`) proving the counters read **real** values (> 0 after a JITted run, distinct under chaining-disabled). **A — the standing profiler:** `bench/profiler/` (a dev tool, not in the .sln — the `bench/hotop-profiler/` successor; the old M6 throwaway is preserved for the `BenchDocReconciliationTests` pin) drives the **real boots** (DOS 3.3, CP/M 2.2, apl2cpm3 CP/M 3.1, Apple Pascal, Spectrum) through the live `BoardMachineFactory`/surface factories + the bench W-kernels (skip-with-note when assets absent), captures both tiers, and emits a versioned, diffable `profile.json` per system×workload under `bench/results/profiles/` + a generated `INDEX.md`. **The first run shipped 17 committed profiles + the FIRST MEASURED ranked backlog.** Key measured findings: the JIT is **slower than the interpreter on every real boot** (DOS 60.8×→23.2× real-time; Spectrum 29.5×→22.6×) — the cold-compile + dispatcher-round-trip floor dominates a short boot (chain:disp ≈ 1.0 DOS / 0.3 Spectrum, far below the bench kernels' good chaining); and the **SoftCard/apl2cpm3 dual-CPU boots never engage the JIT at all** (`Machine.RunDualCpu` drives the primary via `Cpu.Step()`, which `JittedCpu.Step()` routes to the interpreter — a structural finding, not a bug). **Deferred follow-ons (noted):** B (`ips` runtime counter — needs `CpuSpecGenerator` + IL work), D (execution-weighted fallback-by-opcode + emit coverage). Designed in [ADR 0022](architecture/0022-performance-feedback-loop.md). |
...
> **2026-06-23 — ADR 0022 performance feedback loop, FIRST MOVE SHIPPED (items A + C).** The standing
> measurement engine now exists. **C (the free counters):** the JIT exposes four additive, zero-cost
> run-lifetime counters on `IJitMetrics` — `ChainEdgesTaken` / `DispatcherEntries` (the ADR-0012 floor
> signal: chain hops that avoided a dispatcher round-trip vs round-trips taken), `BlockCacheHits` /
> `BlockCacheMisses` — incremented on paths that already run (no per-instruction cost; default behavior
> unchanged; Core stays AOT-clean). **A (the standing profiler):** `bench/profiler/` (a dev tool, not in
> the shipping/test graph — the `bench/hotop-profiler/` successor) drives the **real boots**
> (DOS 3.3, CP/M 2.2, apl2cpm3 CP/M 3.1, Apple Pascal, Spectrum) through the live `BoardMachineFactory`/
> surface factories **plus** the bench W-kernels, captures both tiers (hot-op histogram, the JIT churn +
> chain/dispatch + block-cache counters, real-time ratio, allocations), and emits a versioned, diffable
> `profile.json` per system×workload under `bench/results/profiles/` + a generated `INDEX.md`. **The first
> profile run shipped 17 committed profiles** — the FIRST MEASURED optimization backlog (the data the owner
> asked for; see `bench/results/profiles/INDEX.md`). **Deferred (noted):** **B** (`ips` — the runtime
> retired-instruction counter) needs `CpuSpecGenerator` + emitted-IL work (PR #160's deferral confirmed),
> and **D** (execution-weighted fallback-by-opcode + emit coverage) is the next profiler increment — both
> follow-ons. The conditional arcs (E per-bank, F hot-peripheral, G/L JIT-under-translation) are now
> **evidence-gated** on this profile data rather than assumed. Designed in
> [ADR 0022](architecture/0022-performance-feedback-loop.md).
>
...
5. **[parked 2026-06-23 — owner decision; now evidence-gated by ADR 0022] Per-bank specialization + the generic emitter.** *(Parked — no forcing
   ...
   reveal what generalizes." Revisit (a) if Apple ][+ bank-switch JIT perf becomes a measured bottleneck — **the ADR 0022 profiler now produces that evidence: the first run shows the banked Apple/SoftCard boots have ZERO recompile/eviction churn (no bank-attributable thrash yet), so (a) stays parked on measured grounds, not assumption**.)* (a) **Per-bank `(PC, bankState)` block
