...
> + R (the disk-library catalog) + S (the upload binary path) shipped**: P pushes the REAL machine state — board/asset/video-mode label +
...
> (`window.diskCatalog`/`insertFromLibrary`/`ejectDrive`) the control-strip panel will bind to; S adds the
> surface's first inbound binary path — a binary `DK` upload frame, reassembled across fragments + re-validated
> server-side (`.dsk`/`.po` exact length; `.woz` magic then the honest not-yet-supported reject), loaded into
> drive N via R's `insertDisk` delegate, with the UPLOADING/INSERTED/error state machine + the
> `window.uploadDisk`/`uploadState` client transport the panel will bind to. **The final surface-UI row is T
> (the control-strip UI) — it composes P/R/S** (the drive panels: library select + upload picker + eject + the
> real-motor light + the asset banner); per-PR detail in `docs/BUILDER_QUEUE.md`) — the intended next-up
