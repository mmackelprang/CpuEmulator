...
> a real `Ctrl+B` dropping to BASIC); per-PR detail in `docs/BUILDER_QUEUE.md`. **2026-06-21 — the ZX Spectrum
> 48K ROM UAT (row SU) shipped**: the boot gate recalibrated (`BootCycles` 200k→7M; per-variant 22M for the
> Beckman ROM's coloured-block reset) and parameterized across the owner's **six 48K ROM variants × both tiers**
> (structural mostly-white-paper/black-ink assertion + committed per-variant RGBA hashes), plus an **interactive
> BASIC UAT** that drives the key matrix to type `PRINT 2+2`+ENTER and asserts the printed `4` — boot → keyboard
> → BASIC interpreter → screen end-to-end, both tiers. That live UAT surfaced + fixed a **real production
> interrupt-delivery bug**: `SpectrumUla.OnFrameTick` asserted AND released `/INT` in one synchronous callback,
> so the Z80's level-sampled IM1 was never serviced [the 50 Hz keyboard/`FRAMES` ISR never ran]; the ULA now
> holds `/INT` ~32 T-states via a scheduled release, with a regression gate asserting `FRAMES` ($5C78)
> increments) — the intended next-up sequence, not a delivery commitment. Each item is tagged
