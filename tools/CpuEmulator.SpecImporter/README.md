...
**Provenance — two distinct real references, cross-diffed (a SIMULATED dual-source pass, not a blind
dual-session extraction — read the honesty note):**
...
- **Source B** (`z80-opcodes-b.json`, regenerated by `gen_z80_b.py`): the **clrhome.org Z80 opcode
  table** — a distinct community document with a different format.
- **Honesty note:** the two generators were authored in one session and `gen_z80_b.py` **self-reconciles
  B to agree with A** (its RECONCILIATION block; `RAW=1 python gen_z80_b.py` reproduces the
  un-reconciled raw extraction). This faithfully **validates the diff/reconciliation machinery** — the
  two encoded references are two distinct real documents and the run reproduces the exact 25-cell diff —
  but it is a **SIMULATION of dual-session independence, NOT a true blind second pass**: B was authored
  with knowledge of A, so the two error sets are not genuinely independent. The dataset's correctness
  therefore rests on **single-pass extraction quality** (independently spot-checked: 0 errors across 39
  rows spanning all seven planes) and the **M3.4 TomHarte behavioral gate**, NOT on the cross-diff
  anti-hallucination guarantee. A genuine dual-session blind extraction (two separately-fetched
  documents, transcribed without cross-referencing, B never reconciled-to-A before the diff) is the
  standard for future families (M4/M5) — see the extraction runbook's Rung 2.
